 ...
 *
 * This module implements a noise gate that attenuates audio signals below a specified loudness threshold.
 *
 * Noise Gate Thresholds & Envelope Detection Explanation:
 * 1. Channel Monitoring & Summing:
 *    - Monitored channels are summed together into a scratch buffer to evaluate overall signal loudness.
 * 2. Envelope Detection (Loudness Estimation):
 *    - Instantaneous loudness in dB: val_db = 20.0 * log10(abs(sample) + 1e-9).
 *    - Exponential smoothing filter tracks envelope with first-order IIR attack/release constants:
 ...
 * 4. Gain Application:
 *    - Linear gain multiplier is applied across all processed channels using Apple Accelerate (vDSP)
 *      or scalar fallback.
 * 5. ZERO-ALLOCATION GUARANTEE: Real-time processing (`noise_gate_processor_process`) performs no
...
typedef struct {
    ...
    int* monitor_channels;         ///< Array of channel indices monitored for level detection.
 ...
 * @param params Noise gate parameters (attack, release, threshold, attenuation, etc.).
 * @param sample_rate Audio sample rate in Hz.
 * @param chunk_size Maximum number of frames per processing chunk.
 * @return Pointer to newly allocated noise_gate_processor_t, or NULL on failure.
 */
 ...
 *
 * Evaluates monitored channels, computes envelope loudness and gate threshold gain,
 * and applies linear attenuation to processed channels when gate is closed.
 *
