 ...
 *
 * This implementation provides concrete dispatch tables for each supported audio processor type,
 * routing real-time processing and parameter updates to the appropriate underlying processor
 * (compressor, noise gate, or RACE cross-talk cancellation).
 */
...
static void comp_update(dsp_processor_t* self, const processor_config_t* config, int sample_rate) {
    compressor_processor_update_parameters((compressor_processor_t*)self->impl, config, sample_rate);
}
...
static void comp_free(dsp_processor_t* self) {
    if (self->impl) compressor_processor_free((compressor_processor_t*)self->impl);
    free(self);
}
...
static void gate_update(dsp_processor_t* self, const processor_config_t* config, int sample_rate) {
    noise_gate_processor_update_parameters((noise_gate_processor_t*)self->impl, config, sample_rate);
}
...
static void gate_free(dsp_processor_t* self) {
    if (self->impl) noise_gate_processor_free((noise_gate_processor_t*)self->impl);
    free(self);
}
...
static void race_update(dsp_processor_t* self, const processor_config_t* config, int sample_rate) {
    race_processor_update_parameters((race_processor_t*)self->impl, config, sample_rate);
}
...
dsp_processor_t* dsp_processor_create(const char* name, const processor_config_t* config, int sample_rate, size_t chunk_size) {
    if (!config) return NULL;
    switch (config->type) {
        case PROCESSOR_TYPE_COMPRESSOR: {
            compressor_processor_t* p = compressor_processor_create(name, &config->parameters.compressor, sample_rate, chunk_size);
            return dsp_processor_wrap_compressor(p);
        }
        case PROCESSOR_TYPE_NOISE_GATE: {
            noise_gate_processor_t* p = noise_gate_processor_create(name, &config->parameters.noise_gate, sample_rate, chunk_size);
            return dsp_processor_wrap_noise_gate(p);
        }
        case PROCESSOR_TYPE_RACE: {
            race_processor_t* p = race_processor_create(name, &config->parameters.race, sample_rate);
