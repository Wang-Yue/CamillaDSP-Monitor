...
void silence_counter_init(silence_counter_t* counter, double threshold_db, double timeout_seconds, size_t samplerate, size_t chunksize) {
    if (!counter) return;
    counter->threshold_db = threshold_db;
    counter->silent_chunks = 0;
    if (timeout_seconds > 0.0 && chunksize > 0) {
        counter->limit_chunks = (size_t)round((timeout_seconds * (double)samplerate) / (double)chunksize);
    } else {
...
processing_state_t silence_counter_update(silence_counter_t* counter, double signal_peak_db) {
    ...
    return (counter->silent_chunks >= counter->limit_chunks) ? PROCESSING_STATE_PAUSED : PROCESSING_STATE_RUNNING;
}
