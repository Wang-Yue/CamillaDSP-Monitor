...
engine_state_machine_t* engine_state_machine_create(void) {
    engine_state_machine_t* sm = (engine_state_machine_t*)malloc(sizeof(engine_state_machine_t));
    if (!sm) return NULL;
    atomic_init(&sm->state_raw, processing_state_to_raw_byte(PROCESSING_STATE_INACTIVE));
...
processing_state_t engine_state_machine_get_state(const engine_state_machine_t* sm) {
    if (!sm) return PROCESSING_STATE_INACTIVE;
    uint8_t raw = atomic_load_explicit((_Atomic uint8_t*)&sm->state_raw, memory_order_acquire);
...
void engine_state_machine_set_state(engine_state_machine_t* sm, processing_state_t new_state) {
    if (!sm) return;
...
const processing_stop_reason_t* engine_state_machine_get_stop_reason(const engine_state_machine_t* sm) {
    if (!sm) return NULL;
...
bool engine_state_machine_begin_stop(engine_state_machine_t* sm, processing_stop_reason_t reason) {
    if (!sm) return false;
    bool expected = false;
    bool exchanged = atomic_compare_exchange_strong_explicit(&sm->stop_once, &expected, true, memory_order_acq_rel, memory_order_acquire);
