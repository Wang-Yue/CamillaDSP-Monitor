...
static void mixed_radix_fft_execute_wrapper(void* ctx, waveform_t real_in, waveform_t imag_in, mutable_waveform_t real_out, mutable_waveform_t imag_out, bool inverse) {
    mixed_radix_fft_execute((mixed_radix_fft_t*)ctx, real_in, imag_in, real_out, imag_out, inverse);
}
...
static inline void stage_radix2(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 2;
...
static inline void stage_radix3(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 3;
    double* work_re = fft->work_re;
    double* work_im = fft->work_im;
...
static inline void stage_radix4(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 4;
    double* work_re = fft->work_re;
    double* work_im = fft->work_im;
    for (int b = 0; b < (int)fft->n; b += block_size) {
...
/// arithmetic identity.
static inline void stage_radix5(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 5;
    double* work_re = fft->work_re;
    double* work_im = fft->work_im;
...
static inline void stage_radix7(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 7;
    double* work_re = fft->work_re;
    double* work_im = fft->work_im;
...
static inline void stage_radix8(mixed_radix_fft_t* fft, int m, const double* tw_re, const double* tw_im) {
    int block_size = m * 8;
    double* work_re = fft->work_re;
    double* work_im = fft->work_im;
...
mixed_radix_fft_t* mixed_radix_fft_create(size_t n) {
    ...
    mixed_radix_fft_t* fft = (mixed_radix_fft_t*)calloc(1, sizeof(mixed_radix_fft_t));
    if (!fft) return NULL;
    ...
    if (!fft->factors || !fft->twiddle_re || !fft->twiddle_im || !fft->permutation) {
...
void mixed_radix_fft_execute(mixed_radix_fft_t* fft, waveform_t real_in, waveform_t imag_in, mutable_waveform_t real_out, mutable_waveform_t imag_out, bool inverse) {
    ...
    for (int s = 0; s < fft->stage_count; s++) {
        ...
        switch (r) {
            case 2: stage_radix2(fft, m, twRe, twIm); break;
            case 3: stage_radix3(fft, m, twRe, twIm); break;
            case 4: stage_radix4(fft, m, twRe, twIm); break;
            case 5: stage_radix5(fft, m, twRe, twIm); break;
            case 7: stage_radix7(fft, m, twRe, twIm); break;
            case 8: stage_radix8(fft, m, twRe, twIm); break;
            default: break;
        }
        m *= r;
    }
