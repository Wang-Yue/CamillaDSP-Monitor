...
static inline void __attribute__((unused)) app_logger_sem_init(app_logger_sem_t* sem) { *sem = dispatch_semaphore_create(0); }
static inline void __attribute__((unused)) app_logger_sem_destroy(app_logger_sem_t* sem) { dispatch_release(*sem); }
static inline void __attribute__((unused)) app_logger_sem_signal(app_logger_sem_t* sem) { dispatch_semaphore_signal(*sem); }
static inline void __attribute__((unused)) app_logger_sem_wait(app_logger_sem_t* sem) { dispatch_semaphore_wait(*sem, DISPATCH_TIME_FOREVER); }
#else
#include <semaphore.h>
typedef sem_t app_logger_sem_t;
static inline void __attribute__((unused)) app_logger_sem_init(app_logger_sem_t* sem) { sem_init(sem, 0, 0); }
static inline void __attribute__((unused)) app_logger_sem_destroy(app_logger_sem_t* sem) { sem_destroy(sem); }
static inline void __attribute__((unused)) app_logger_sem_signal(app_logger_sem_t* sem) { sem_post(sem); }
static inline void __attribute__((unused)) app_logger_sem_wait(app_logger_sem_t* sem) { sem_wait(sem); }
#endif
...
log_level_t app_logger_get_level(void) {
    return (log_level_t)atomic_load_explicit(&g_current_log_level, memory_order_acquire);
}

void app_logger_set_level(log_level_t level) {
    atomic_store_explicit(&g_current_log_level, (uint8_t)level, memory_order_release);
}

static void format_log_message(char* out, size_t out_cap, const char* msg, const log_argument_t args[4]) {
        ...
        if (strchr("diouxXcp", conv)) {
            if (arg.type == LOG_ARG_INT) {
                if (conv == 'c') {
                    ...
                    char fmt[64];
                    size_t flen = 0;
                    for (const char* s = spec_start; s < q && flen < sizeof(fmt) - 5; s++) {
                        if (!strchr("hljztqL", *s)) {
                            fmt[flen++] = *s;
                        }
                    }
    ...
    for (; arg_idx < 4; arg_idx++) {
        ...
        switch (args[arg_idx].type) {
            case LOG_ARG_NONE: break;
            case LOG_ARG_INT: snprintf(tmp, sizeof(tmp), " %lld", (long long)args[arg_idx].val.i); break;
            case LOG_ARG_DOUBLE: snprintf(tmp, sizeof(tmp), " %.6f", args[arg_idx].val.d); break;
            case LOG_ARG_STRING: snprintf(tmp, sizeof(tmp), " %s", args[arg_idx].val.s ? args[arg_idx].val.s : "(null)"); break;
        }
        for (const char* s = tmp; *s != '\0'; s++) {
            if (out_len + 1 < out_cap) {
                out[out_len++] = *s;
...
static void* worker_thread_func(void* arg) {
    ...
    while (!atomic_load_explicit(&logger->should_exit, memory_order_acquire)) {
        ...
        while (true) {
            uint64_t r = atomic_load_explicit(&logger->read_index, memory_order_relaxed);
            uint64_t w = atomic_load_explicit(&logger->write_index, memory_order_acquire);
            if (r == w) break;
            ...
            switch (rec.level) {
                case LOG_LEVEL_OFF: lvl_str = "OFF"; break;
                ...
                case LOG_LEVEL_TRACE: lvl_str = "TRACE"; break;
            }
            char formatted_msg[2048];
            log_argument_t args[4] = { rec.arg1, rec.arg2, rec.arg3, rec.arg4 };
            format_log_message(formatted_msg, sizeof(formatted_msg), rec.message, args);
            printf("[%s] %s: %s\n", lvl_str, rec.label ? rec.label : "", formatted_msg);
        }
    }
    return NULL;
}
...
static void init_shared_logger(void) {
    // Intentionally empty/default-init to guarantee safe singleton instance publication before thread activation.
    ...
    g_shared_logger->storage = (log_record_t*)calloc(g_shared_logger->capacity, sizeof(log_record_t));
...
void app_logger_log(
    ...
    bool expected = false;
    if (atomic_compare_exchange_strong_explicit(&logger->is_started, &expected, true, memory_order_acq_rel, memory_order_acquire)) {
