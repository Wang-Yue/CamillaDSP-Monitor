...
int core_audio_device_all_ids(AudioDeviceID* out_ids, int max_ids) {
    ...
    if (out_ids && count > 0) {
        ...
        if (AudioObjectGetPropertyData(kAudioObjectSystemObject, &addr, 0, NULL, &size, out_ids) != noErr) {
            return 0;
        }
    }
    return count;
...
bool core_audio_device_name(AudioDeviceID device_id, char* out_name, size_t max_len) {
    ...
    if (AudioObjectGetPropertyData(device_id, &addr, 0, NULL, &size, &cf_name) == noErr && cf_name) {
        CFStringGetCString(cf_name, out_name, (CFIndex)max_len, kCFStringEncodingUTF8);
        CFRelease(cf_name);
        return true;
    }
    return false;
...
bool core_audio_device_has_stream(AudioDeviceID device_id, core_audio_scope_t scope) {
    AudioObjectPropertyAddress addr = {
        .mSelector = kAudioDevicePropertyStreams,
        .mScope = (scope == CORE_AUDIO_SCOPE_INPUT) ? kAudioDevicePropertyScopeInput : kAudioDevicePropertyScopeOutput,
        .mElement = kAudioObjectPropertyElementMain
    };
    uint32_t size = 0;
    OSStatus status = AudioObjectGetPropertyDataSize(device_id, &addr, 0, NULL, &size);
    return (status == noErr && size >= sizeof(AudioStreamID));
}

/// HAL stream IDs for the given device + direction.
int core_audio_device_streams(AudioDeviceID device_id, core_audio_scope_t scope, AudioStreamID* out_streams, int max_streams) {
    AudioObjectPropertyAddress addr = {
        .mSelector = kAudioDevicePropertyStreams,
        .mScope = (scope == CORE_AUDIO_SCOPE_INPUT) ? kAudioDevicePropertyScopeInput : kAudioDevicePropertyScopeOutput,
        .mElement = kAudioObjectPropertyElementMain
    ...
    uint32_t size = 0;
    if (AudioObjectGetPropertyDataSize(device_id, &addr, 0, NULL, &size) != noErr || size == 0) {
        return 0;
    }
    ...
    if (out_streams && count > 0) {
        ...
        if (AudioObjectGetPropertyData(device_id, &addr, 0, NULL, &size, out_streams) != noErr) {
            return 0;
        }
    }
    return count;
...
int core_audio_device_list_devices(core_audio_scope_t scope, core_audio_device_info_t* out_devices, int max_devices) {
    ...
    for (int i = 0; i < total && count < max_devices; i++) {
        if (core_audio_device_has_stream(all_ids[i], scope)) {
            if (out_devices) {
                ...
                if (!core_audio_device_name(all_ids[i], out_devices[count].name, sizeof(out_devices[count].name))) {
...
AudioDeviceID core_audio_device_default_id(core_audio_scope_t scope) {
    AudioObjectPropertyAddress addr = {
        .mSelector = (scope == CORE_AUDIO_SCOPE_INPUT) ? kAudioHardwarePropertyDefaultInputDevice : kAudioHardwarePropertyDefaultOutputDevice,
        .mScope = kAudioObjectPropertyScopeGlobal,
        .mElement = kAudioObjectPropertyElementMain
    };
    AudioDeviceID id = 0;
...
AudioDeviceID core_audio_device_id_for_name(const char* name, core_audio_scope_t scope) {
    if (!name || name[0] == '\0') {
...
bool core_audio_device_get_nominal_sample_rate(AudioDeviceID device_id, double* out_rate) {
    AudioObjectPropertyAddress addr = {
    ...
    if (AudioObjectGetPropertyData(device_id, &addr, 0, NULL, &size, &rate) == noErr) {
...
/// without polling.
bool core_audio_device_set_nominal_sample_rate(AudioDeviceID device_id, double rate) {
    AudioObjectPropertyAddress addr = {
        .mSelector = kAudioDevicePropertyNominalSampleRate,
        .mScope = kAudioObjectPropertyScopeGlobal,
        .mElement = kAudioObjectPropertyElementMain
    ...
    Float64 value = rate;
    OSStatus status = AudioObjectSetPropertyData(device_id, &addr, 0, NULL, sizeof(Float64), &value);
...
/// Set the device's buffer frame size for a given scope. Returns `true` on success.
bool core_audio_device_set_buffer_frame_size(AudioDeviceID device_id, uint32_t frames, core_audio_scope_t scope) {
    AudioObjectPropertyAddress addr = {
    ...
    return (AudioObjectSetPropertyData(device_id, &addr, 0, NULL, sizeof(uint32_t), &value) == noErr);
}

/// Read the device's current buffer frame size for a given scope.
bool core_audio_device_get_buffer_frame_size(AudioDeviceID device_id, core_audio_scope_t scope, uint32_t* out_frames) {
    AudioObjectPropertyAddress addr = {
        .mSelector = kAudioDevicePropertyBufferFrameSize,
        .mScope = (scope == CORE_AUDIO_SCOPE_INPUT) ? kAudioDevicePropertyScopeInput : kAudioDevicePropertyScopeOutput,
        .mElement = kAudioObjectPropertyElementMain
    ...
    if (AudioObjectGetPropertyData(device_id, &addr, 0, NULL, &size, &frames) == noErr) {
...
bool core_audio_device_set_clock_source_id(AudioDeviceID device_id, uint32_t source_id) {
    AudioObjectPropertyAddress addr = {
...
bool core_audio_device_select_adjustable_clock_source(AudioDeviceID device_id) {
    ...
    if (AudioObjectGetPropertyData(device_id, &addr, 0, NULL, &size, ids) != noErr) {
        return false;
    }
    for (int i = 0; i < count; i++) {
        ...
        AudioValueTranslation trans = {
            .mInputData = &source_id,
            .mInputDataSize = sizeof(uint32_t),
            .mOutputData = &cf_name,
            .mOutputDataSize = sizeof(CFStringRef)
        };
        uint32_t trans_size = sizeof(AudioValueTranslation);
        if (AudioObjectGetPropertyData(device_id, &name_addr, 0, NULL, &trans_size, &trans) == noErr && cf_name) {
            char name_buf[256];
            if (CFStringGetCString(cf_name, name_buf, sizeof(name_buf), kCFStringEncodingUTF8)) {
...
bool core_audio_device_has_nominal_sample_rate_property(AudioDeviceID device_id) {
    AudioObjectPropertyAddress addr = {
        .mSelector = kAudioDevicePropertyNominalSampleRate,
        .mScope = kAudioObjectPropertyScopeGlobal,
        .mElement = kAudioObjectPropertyElementMain
...
AudioStreamBasicDescription core_audio_device_float32_stream_format(double sample_rate, int channels, bool interleaved) {
...
static OSStatus rate_change_listener_callback(AudioObjectID inObjectID, UInt32 inNumberAddresses, const AudioObjectPropertyAddress* inAddresses, void* inClientData) {
    ...
    if (core_audio_device_get_nominal_sample_rate(inObjectID, &rate)) {
        if (rate > 0.0) {
            ...
            atomic_store_explicit(&watcher->latest_rate_bits, bits, memory_order_release);
        }
    }
    return noErr;
}

rate_change_watcher_t* rate_change_watcher_create(AudioDeviceID device_id, double expected_rate) {
    rate_change_watcher_t* watcher = (rate_change_watcher_t*)calloc(1, sizeof(rate_change_watcher_t));
    ...
    OSStatus status = AudioObjectAddPropertyListener(device_id, &addr, rate_change_listener_callback, watcher);
    if (status != noErr) {
        free(watcher);
        return NULL;
    }
...
bool rate_change_watcher_get_pending_change(rate_change_watcher_t* watcher, double* out_rate) {
    if (!watcher) return false;
    uint64_t bits = atomic_load_explicit(&watcher->latest_rate_bits, memory_order_acquire);
...
void rate_change_watcher_dispose(rate_change_watcher_t* watcher) {
    ...
    if (atomic_load_explicit(&watcher->registered, memory_order_acquire)) {
        ...
        AudioObjectRemovePropertyListener(watcher->device_id, &addr, rate_change_listener_callback, watcher);
