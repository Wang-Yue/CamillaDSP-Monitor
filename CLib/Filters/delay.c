...
static void build_delay(double delay_samples, bool subsample, int* out_integer_delay, biquad_coefficients_t* out_coeffs, bool* out_has_coeffs) {
    *out_has_coeffs = false;
    if (subsample) {
        ...
        double coeff2 = ((2.0 - fraction) / (2.0 + fraction)) * ((1.0 - fraction) / (1.0 + fraction));
...
delay_filter_t* delay_filter_create(const char* name, const delay_parameters_t* params, int sample_rate) {
...
void delay_filter_process(delay_filter_t* filter, mutable_waveform_t waveform, size_t count) {
...
void delay_filter_update_parameters(delay_filter_t* filter, const filter_config_t* config, int sample_rate) {
    if (!filter || !config) return;
    if (config->type != FILTER_TYPE_DELAY) return;
    const delay_parameters_t* params = &config->parameters.delay;

    double delay_samples = compute_delay_samples(params->delay, params->unit, sample_rate);
    int integer_delay = 0;
    biquad_coefficients_t coeffs;
    bool has_coeffs = false;
    build_delay(delay_samples, params->subsample, &integer_delay, &coeffs, &has_coeffs);
