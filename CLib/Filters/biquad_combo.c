...
static biquad_filter_t* create_section(biquad_type_t type, double freq, double q, double gain, double slope, double bandwidth, int sample_rate) {
...
biquad_combo_filter_t* biquad_combo_filter_create(const char* name, const biquad_combo_parameters_t* params, int sample_rate) {
    ...
    biquad_combo_filter_t* filter = (biquad_combo_filter_t*)malloc(sizeof(biquad_combo_filter_t));
    ...
    case BIQUAD_COMBO_TYPE_BUTTERWORTH_HIGHPASS: {
        ...
        for (size_t i = 0; i < nq; i++) {
            ...
            secs[num++] = create_section(t, params->freq, q_vals[i] > 0 ? q_vals[i] : 0.707, 0.0, 0.0, 0.0, sample_rate);
        }
        break;
    }
    ...
    case BIQUAD_COMBO_TYPE_LINKWITZ_RILEY_HIGHPASS: {
        ...
        for (size_t i = 0; i < nq; i++) {
            ...
            secs[num++] = create_section(t, params->freq, q_vals[i], 0.0, 0.0, 0.0, sample_rate);
        }
        break;
    }
    // MARK: - Tilt EQ
    case BIQUAD_COMBO_TYPE_TILT: {
        ...
        secs[num++] = create_section(BIQUAD_TYPE_LOWSHELF, 110.0, 0.35, -gain / 2.0, 0.0, 0.0, sample_rate);
        secs[num++] = create_section(BIQUAD_TYPE_HIGHSHELF, 3500.0, 0.35, gain / 2.0, 0.0, 0.0, sample_rate);
        break;
    }
    // MARK: - Graphic EQ
    case BIQUAD_COMBO_TYPE_GRAPHIC_EQUALIZER: {
        ...
        for (size_t i = 0; i < nb; i++) {
            ...
            secs[num++] = create_section(BIQUAD_TYPE_PEAKING, f, 0.0, g, 0.0, bw, sample_rate);
        }
        break;
    }
    // MARK: - Five Point PEQ
    case BIQUAD_COMBO_TYPE_FIVE_POINT_PEQ: {
        // Low shelf
        secs[num++] = create_section(BIQUAD_TYPE_LOWSHELF, params->fls > 0 ? params->fls : 80.0, params->qls > 0 ? params->qls : 0.707, params->gls, 0.0, 0.0, sample_rate);
        // Mid bands
        secs[num++] = create_section(BIQUAD_TYPE_PEAKING, params->fp1 > 0 ? params->fp1 : 250.0, params->qp1 > 0 ? params->qp1 : 1.5, params->gp1, 0.0, 0.0, sample_rate);
        secs[num++] = create_section(BIQUAD_TYPE_PEAKING, params->fp2 > 0 ? params->fp2 : 1000.0, params->qp2 > 0 ? params->qp2 : 2.0, params->gp2, 0.0, 0.0, sample_rate);
        secs[num++] = create_section(BIQUAD_TYPE_PEAKING, params->fp3 > 0 ? params->fp3 : 4000.0, params->qp3 > 0 ? params->qp3 : 1.0, params->gp3, 0.0, 0.0, sample_rate);
        // High shelf
        secs[num++] = create_section(BIQUAD_TYPE_HIGHSHELF, params->fhs > 0 ? params->fhs : 12000.0, params->qhs > 0 ? params->qhs : 0.707, params->ghs, 0.0, 0.0, sample_rate);
        break;
    }
    }
...
void biquad_combo_filter_update_parameters(biquad_combo_filter_t* filter, const filter_config_t* config, int sample_rate) {
    ...
    biquad_combo_filter_t* tmp = biquad_combo_filter_create(filter->name, &config->parameters.biquad_combo, sample_rate);
    if (tmp) {
