...
/**
 * @brief Populates the internal routing matrix mapping from a mixer configuration.
 *
 * Precomputes linear gains and channel routing lists for all destination channels.
 *
 * @param mixer Pointer to audio mixer instance.
 * @param config Configuration containing mapping rules.
 */
static void populate_mapping(audio_mixer_t* mixer, const mixer_config_t* config) {
    for (size_t i = 0; i < config->mapping_count; i++) {
        const mixer_mapping_t* map = &config->mapping[i];
        size_t dest = (size_t)map->dest;
        // Ignore mappings to out-of-bounds destination channels or muted destination mappings
        ...
        mixer->mapping[dest].sources = (prepared_source_t*)malloc(valid_count * sizeof(prepared_source_t));
...
audio_mixer_t* audio_mixer_create(const char* name, const mixer_config_t* config, size_t chunk_size) {
    ...
    mixer->mapping = (prepared_source_list_t*)calloc(mixer->channels_out, sizeof(prepared_source_list_t));
    if (!mixer->mapping) {
        free(mixer->name);
        free(mixer);
        return NULL;
...
audio_chunk_t* audio_mixer_process_chunk(audio_mixer_t* mixer, const audio_chunk_t* input) {
    ...
    audio_chunk_t* output = audio_chunk_create(input->valid_frames, mixer->channels_out);
