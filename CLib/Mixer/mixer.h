 ...
 *
 * This module implements an audio mixer that changes channel counts and routes/sums
 * audio between channels according to a configurable routing matrix.
 *
 * Channel Routing Matrix Explanation:
 * - A routing matrix defines mapping rules from source channels (input) to destination channels (output).
 * - Each destination channel maintains a list of prepared sources that contribute to its output.
 * - For each source channel contributing to a destination:
 *   - The gain can be specified in linear scale or decibels (dB). If specified in dB, it is
 *     converted to linear gain: lin_gain = 10^(gain_db / 20).
 *   - If the source is marked as inverted, the linear gain is multiplied by -1.0 to invert phase.
 *   - Muted sources or destination mappings are excluded from processing.
 * - During processing, destination channel buffers are cleared to zero and source channels are
 *   summed in: if gain is 1.0, direct sample addition (add) is performed; otherwise, multiply-add
 *   is performed using Apple Accelerate (vDSP) or scalar fallback.
 * - ZERO-ALLOCATION GUARANTEE: Real-time audio processing (`audio_mixer_process`) performs no
 *   memory allocations or deallocations. All mapping tables and buffers are prepared during
...
/**
 * @brief Represents a prepared source channel contribution to a destination channel.
 */
typedef struct {
...
/**
 * @brief List of prepared source contributions for a single destination channel.
 */
typedef struct {
...
typedef struct {
    ...
    prepared_source_list_t* mapping; ///< Array of length channels_out defining source routing.
} audio_mixer_t;
 ...
 * @param config Mixer configuration containing channel counts and mapping matrix.
 ...
 */
audio_mixer_t* audio_mixer_create(const char* name, const mixer_config_t* config, size_t chunk_size);
...
mixer_error_t audio_mixer_process(audio_mixer_t* mixer, const audio_chunk_t* input, audio_chunk_t* output);
 ...
 * Note: This function allocates memory and should not be used on real-time audio threads.
 *
 ...
 */
audio_chunk_t* audio_mixer_process_chunk(audio_mixer_t* mixer, const audio_chunk_t* input);

/**
 * @brief Updates mixer routing parameters dynamically.
 *
 * Re-evaluates the mapping matrix from the configuration and updates internal prepared sources.
 *
 * @param mixer Pointer to audio mixer instance.
 * @param config New mixer configuration.
 */
void audio_mixer_update_parameters(audio_mixer_t* mixer, const mixer_config_t* config);
