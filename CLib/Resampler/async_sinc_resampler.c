...
async_sinc_resampler_t* async_sinc_resampler_create(size_t channels, size_t input_rate, size_t output_rate, size_t sinc_len, size_t oversampling_factor, sinc_interpolation_type_t interpolation, window_function_t window, double f_cutoff, bool has_f_cutoff, size_t chunk_size, double max_relative_ratio) {
    ...
    async_sinc_resampler_t* resampler = (async_sinc_resampler_t*)calloc(1, sizeof(async_sinc_resampler_t));
    ...
    float base_cutoff = has_f_cutoff ? (float)f_cutoff : calculate_cutoff_f32(sinc_len, window);
    float fc_f32 = resampler->base_ratio >= 1.0 ? base_cutoff : base_cutoff * (float)resampler->base_ratio;
    double fc = (double)fc_f32;

    resampler->sinc_table = make_sinc_table(sinc_len, oversampling_factor, window, fc);
    ...
    double raw_max = ((double)chunk_size - (double)(sinc_len + 1) - most_neg_last_index) * max_ratio_abs;
    resampler->max_output_frames = (size_t)(ceil(raw_max)) + 16;

    // Pre-allocate scratch for per-frame state.
    resampler->idx_scratch = (double*)calloc(resampler->max_output_frames, sizeof(double));
    resampler->frac_scratch = (double*)calloc(resampler->max_output_frames, sizeof(double));
...
async_sinc_resampler_t* async_sinc_resampler_create_from_profile(size_t channels, size_t input_rate, size_t output_rate, resampler_profile_t profile, size_t chunk_size, double max_relative_ratio) {
    ...
    switch (profile) {
        case RESAMPLER_PROFILE_VERY_FAST:
            sinc_len = 64; oversampling_factor = 1024; window = WINDOW_FUNCTION_HANN2; interpolation = SINC_INTERPOLATION_LINEAR;
            break;
        case RESAMPLER_PROFILE_FAST:
            sinc_len = 128; oversampling_factor = 1024; window = WINDOW_FUNCTION_BLACKMAN2; interpolation = SINC_INTERPOLATION_LINEAR;
            break;
        case RESAMPLER_PROFILE_BALANCED:
            sinc_len = 192; oversampling_factor = 512; window = WINDOW_FUNCTION_BLACKMAN_HARRIS2; interpolation = SINC_INTERPOLATION_QUADRATIC;
            break;
        case RESAMPLER_PROFILE_ACCURATE:
            sinc_len = 256; oversampling_factor = 256; window = WINDOW_FUNCTION_BLACKMAN_HARRIS2; interpolation = SINC_INTERPOLATION_CUBIC;
            break;
        default:
            break;
    }

    return async_sinc_resampler_create(channels, input_rate, output_rate, sinc_len, oversampling_factor, interpolation, window, 0.0, false, chunk_size, max_relative_ratio);
}
...
static inline size_t get_next_output_frames(const async_sinc_resampler_t* resampler) {
    // Calculate output size for input
    // — note `.floor()`, not `.ceil()`. Using ceil
    // here was the source of the off-by-one frame discrepancy.
    double avg_ratio = 0.5 * resampler->resample_ratio + 0.5 * resampler->target_ratio;
    double raw = ((double)resampler->chunk_size - (double)(resampler->sinc_len + 1) - resampler->last_index) * avg_ratio;
...
/// Wrap-around logic.
static inline adjust_point_t adjust_point(int start, int frac, int sub, int factor) {
    int index = start;
...
static void run_cubic(async_sinc_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        for (size_t frame = 0; frame < output_frames; frame++) {
            ...
            double p0 = sinc_dot_product(buf + p0t.idx + two_s_len, table + p0t.sub * s_len, s_len);
            double p1 = sinc_dot_product(buf + p1t.idx + two_s_len, table + p1t.sub * s_len, s_len);
            double p2 = sinc_dot_product(buf + p2t.idx + two_s_len, table + p2t.sub * s_len, s_len);
            double p3 = sinc_dot_product(buf + p3t.idx + two_s_len, table + p3t.sub * s_len, s_len);
...
static void run_quadratic(async_sinc_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
    size_t s_len = resampler->sinc_len;
    size_t two_s_len = 2 * s_len;
    int factor = (int)resampler->oversampling_factor;
    double factor_d = (double)factor;
...
static void run_linear(async_sinc_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
    size_t s_len = resampler->sinc_len;
    size_t two_s_len = 2 * s_len;
    int factor = (int)resampler->oversampling_factor;
    double factor_d = (double)factor;
...
resampler_error_t async_sinc_resampler_process(async_sinc_resampler_t* resampler, const audio_chunk_t* input, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        memcpy(dst_ptr + two_s_len, src_ptr, resampler->chunk_size * sizeof(double));
    }
    ...
    double t_ratio_increment = (t_ratio_end - t_ratio_start) / (double)output_frames;
