...
synchronous_resampler_t* synchronous_resampler_create(size_t channels, size_t input_rate, size_t output_rate, size_t requested_chunk_size) {
    if (channels == 0 || requested_chunk_size == 0 || input_rate == 0 || output_rate == 0) return NULL;

    synchronous_resampler_t* resampler = (synchronous_resampler_t*)calloc(1, sizeof(synchronous_resampler_t));
    ...
    resampler->shared_bins = (input_block < output_block ? input_block : output_block) + 1;
    ...
    if (input_rate > output_rate) {
        double base_cut = calculate_cutoff(output_block, WINDOW_FUNCTION_BLACKMAN_HARRIS2);
    ...
    double* kernel = make_sinc_table(input_block, 1, WINDOW_FUNCTION_BLACKMAN_HARRIS2, cutoff);
    if (!kernel) {
        free(resampler);
        return NULL;
    }
    ...
    real_fft_forward(resampler->input_fft, filter_time, resampler->filter_spec_re, resampler->filter_spec_im);
    free(filter_time);
    ...
    if (!resampler->working_time || !resampler->working_spec_re || !resampler->working_spec_im) {
...
resampler_error_t synchronous_resampler_process(synchronous_resampler_t* resampler, const audio_chunk_t* input, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        memcpy(resampler->working_time, src_ptr, resampler->input_block_len * sizeof(double));
        memset(resampler->working_time + resampler->input_block_len, 0, resampler->input_block_len * sizeof(double));

        // Step 2. Forward 2N-point real FFT.
        real_fft_forward(resampler->input_fft, resampler->working_time, resampler->working_spec_re, resampler->working_spec_im);
...
#ifdef __APPLE__
        DSPDoubleSplitComplex io_split = {resampler->working_spec_re, resampler->working_spec_im};
        DSPDoubleSplitComplex f_split = {resampler->filter_spec_re, resampler->filter_spec_im};
        vDSP_zvmulD(&io_split, 1, &f_split, 1, &io_split, 1, resampler->shared_bins, 1);
#else
        ...
        if (resampler->output_block_len + 1 > resampler->shared_bins) {
            size_t zero_count = resampler->output_block_len + 1 - resampler->shared_bins;
            memset(resampler->working_spec_re + resampler->shared_bins, 0, zero_count * sizeof(double));
            memset(resampler->working_spec_im + resampler->shared_bins, 0, zero_count * sizeof(double));
        }

        // Step 5. Inverse 2P-point real FFT to time domain (P = outputBlockLen).
        real_fft_inverse(resampler->output_fft, resampler->working_spec_re, resampler->working_spec_im, resampler->working_time);

        // Step 6. Overlap-add: write `result[0..P) + carry` as the chunk's
        // output samples, and save `result[P..2P)` for the next chunk's
        // overlap.
#ifdef __APPLE__
        vDSP_vaddD(resampler->working_time, 1, carry_ptr, 1, out_ptr, 1, resampler->output_block_len);
#else
        for (size_t i = 0; i < resampler->output_block_len; i++) {
            out_ptr[i] = resampler->working_time[i] + carry_ptr[i];
        }
#endif
        memcpy(carry_ptr, resampler->working_time + resampler->output_block_len, resampler->output_block_len * sizeof(double));
    }
