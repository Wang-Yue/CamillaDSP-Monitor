...
#define ASSERT_EQ(exp, act) do { \
    if ((exp) != (act)) { \
        printf("  [FAIL] %s:%d: Expected %s == %s (got %lld vs %lld).\n", __FILE__, __LINE__, #exp, #act, (long long)(exp), (long long)(act)); \
        g_test_failures++; \
        return; \
    } \
} while(0)

#define ASSERT_NE(exp, act) do { \
    if ((exp) == (act)) { \
