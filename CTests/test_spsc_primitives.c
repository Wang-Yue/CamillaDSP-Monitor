...
#include "../CLib/Audio/lock_free_ring_buffer.h"
#include <pthread.h>
#include <stdatomic.h>
#include <math.h>
...
static void* spmc_producer_thread(void* arg) {
    ...
    while (written < a->total_to_write) {
        ...
        spsc_audio_ring_buffer_append_converting_double_to_float(a->ring, chunk, a->chunk_size);
...
TEST(ConcurrentProducerConsumerSeesMonotonicSequence) {
    ...
    ASSERT_TRUE(spsc_audio_ring_buffer_get_total_samples_written(ring) >= (uint64_t)total_to_write);
    spsc_audio_ring_buffer_free(ring);
}
