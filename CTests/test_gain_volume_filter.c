...
TEST(GainInvert) {
    gain_parameters_t params = { .gain = 0.0, .has_gain = true, .scale = GAIN_SCALE_DB, .inverted = true };
    gain_filter_t* filter = gain_filter_create("gain", &params);
    ASSERT_TRUE(filter != NULL);
    prc_fmt_t waveform[] = {-0.5, 0.0, 0.5};
...
TEST(GainAmplify) {
    gain_parameters_t params = { .gain = 20.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    gain_filter_t* filter = gain_filter_create("gain", &params);
    ASSERT_TRUE(filter != NULL);
    prc_fmt_t waveform[] = {-0.5, 0.0, 0.5};
    gain_filter_process(filter, waveform, 3);
...
TEST(GainMute) {
    ...
    gain_filter_t* filter = gain_filter_create("gain", &params);
    ASSERT_TRUE(filter != NULL);
    prc_fmt_t waveform[] = {-0.5, 0.0, 0.5, 1.0, -1.0};
...
TEST(GainLinearScale) {
    gain_parameters_t params = { .gain = 0.5, .has_gain = true, .scale = GAIN_SCALE_LINEAR };
    gain_filter_t* filter = gain_filter_create("gain", &params);
    ASSERT_TRUE(filter != NULL);
    prc_fmt_t waveform[] = {1.0};
    gain_filter_process(filter, waveform, 1);
    ASSERT_NEAR(0.5, waveform[0], 1e-10);
    gain_filter_free(filter);
}

static void process_vol(volume_filter_t* filter, prc_fmt_t* waveform, size_t count) {
...
TEST(VolumeUnityGain) {
    ...
    processing_parameters_set_target_volume_for_fader(proc_params, 0.0, FADER_MAIN);
    volume_parameters_t params = { .ramp_time = 0.0, .has_ramp_time = true, .limit = 50.0, .has_limit = true, .fader = FADER_MAIN };
    volume_filter_t* filter = volume_filter_create("volume", &params, 44100, 4, proc_params);
    ASSERT_TRUE(filter != NULL);

    prc_fmt_t waveform[] = {1.0, -0.5, 0.25, 0.0};
    prc_fmt_t original[] = {1.0, -0.5, 0.25, 0.0};
...
TEST(VolumeAttenuation) {
    processing_parameters_t* proc_params = processing_parameters_create(2, 2);
    processing_parameters_set_target_volume_for_fader(proc_params, -20.0, FADER_MAIN);
    volume_parameters_t params = { .ramp_time = 0.0, .has_ramp_time = true, .limit = 50.0, .has_limit = true, .fader = FADER_MAIN };
    volume_filter_t* filter = volume_filter_create("volume", &params, 44100, 4, proc_params);
    ASSERT_TRUE(filter != NULL);

    prc_fmt_t waveform[] = {1.0, -1.0, 0.5, -0.5};
    process_vol(filter, waveform, 4);
    double gain = prc_fmt_from_db(-20.0);
...
TEST(VolumeMuteRampsToZero) {
    ...
    prc_fmt_t waveform[] = {1.0, 0.5, -0.5, -1.0};
    process_vol(filter, waveform, 4);
    for (size_t i = 0; i < 4; i++) {
...
TEST(VolumeRamp) {
    ...
    processing_parameters_t* proc_params = processing_parameters_create(2, 2);
    processing_parameters_set_target_volume_for_fader(proc_params, 0.0, FADER_MAIN);
    volume_parameters_t params = { .ramp_time = ramp_time_ms, .has_ramp_time = true, .limit = 50.0, .has_limit = true, .fader = FADER_MAIN };
    volume_filter_t* filter = volume_filter_create("volume", &params, sample_rate, chunk_size, proc_params);
    ASSERT_TRUE(filter != NULL);

    prc_fmt_t chunk0[] = {1.0, 1.0, 1.0, 1.0};
    ...
    prc_fmt_t chunk1[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, chunk1, 4);

    double gain0db = prc_fmt_from_db(0.0);
    double gain_m20db = prc_fmt_from_db(-20.0);
    for (size_t i = 0; i < chunk_size; i++) {
    ...
    prc_fmt_t chunk2[] = {1.0, 1.0, 1.0, 1.0};
    ...
    prc_fmt_t chunk3[] = {1.0, 1.0, 1.0, 1.0};
...
TEST(VolumeChangeThreshold) {
    ...
    prc_fmt_t wave1[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, wave1, 4);

    processing_parameters_set_target_volume_for_fader(proc_params, 0.005, FADER_MAIN);
    prc_fmt_t wave2[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, wave2, 4);
    for (size_t i = 0; i < 4; i++) {
        ASSERT_NEAR(1.0, wave2[i], 1e-10);
    }

    processing_parameters_set_target_volume_for_fader(proc_params, 0.02, FADER_MAIN);
    prc_fmt_t wave3[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, wave3, 4);
    double expected_gain = prc_fmt_from_db(0.02);
    for (size_t i = 0; i < 4; i++) {
...
TEST(VolumeLimit) {
    processing_parameters_t* proc_params = processing_parameters_create(2, 2);
    processing_parameters_set_target_volume_for_fader(proc_params, 0.0, FADER_MAIN);
    volume_parameters_t params = { .ramp_time = 0.0, .has_ramp_time = true, .limit = 10.0, .has_limit = true, .fader = FADER_MAIN };
    volume_filter_t* filter = volume_filter_create("volume", &params, 44100, 4, proc_params);
    ASSERT_TRUE(filter != NULL);

    processing_parameters_set_target_volume_for_fader(proc_params, 20.0, FADER_MAIN);
    prc_fmt_t waveform[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, waveform, 4);

    double expected_gain = prc_fmt_from_db(10.0);
    for (size_t i = 0; i < 4; i++) {
...
TEST(VolumeUpdateParametersClampsToLimit) {
    ...
    processing_parameters_set_target_volume_for_fader(proc_params, 10.0, FADER_MAIN);
    prc_fmt_t wave2[] = {1.0, 1.0, 1.0, 1.0};
    process_vol(filter, wave2, 4);
