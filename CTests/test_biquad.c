...
static void gain_and_phase(const biquad_coefficients_t* coeffs, double f, double fs, double* gain_db, double* phase_deg) {
...
static biquad_coefficients_t make_coeffs(biquad_parameters_t* params, int sample_rate) {
...
TEST(LowshelfSlopeVsQ) {
    biquad_parameters_t pS = { .type = BIQUAD_TYPE_LOWSHELF, .freq = 100.0, .gain = -24.0, .slope = 12.0 };
    biquad_parameters_t pQ = { .type = BIQUAD_TYPE_LOWSHELF, .freq = 100.0, .q = 1.0 / sqrt(2.0), .gain = -24.0 };
...
TEST(HighshelfSlopeVsQ) {
    biquad_parameters_t pS = { .type = BIQUAD_TYPE_HIGHSHELF, .freq = 100.0, .gain = -24.0, .slope = 12.0 };
    biquad_parameters_t pQ = { .type = BIQUAD_TYPE_HIGHSHELF, .freq = 100.0, .q = 1.0 / sqrt(2.0), .gain = -24.0 };
    biquad_coefficients_t cS = make_coeffs(&pS, 44100);
    biquad_coefficients_t cQ = make_coeffs(&pQ, 44100);
    ASSERT_TRUE(is_close_relative(cS.a1, cQ.a1, 0.001));
    ASSERT_TRUE(is_close_relative(cS.a2, cQ.a2, 0.001));
...
TEST(BandpassBWvsQ) {
    biquad_parameters_t pBW = { .type = BIQUAD_TYPE_BANDPASS, .freq = 100.0, .bandwidth = 1.0 };
    biquad_parameters_t pQ = { .type = BIQUAD_TYPE_BANDPASS, .freq = 100.0, .q = sqrt(2.0) };
...
TEST(NotchBWvsQ) {
    biquad_parameters_t pBW = { .type = BIQUAD_TYPE_NOTCH, .freq = 100.0, .bandwidth = 1.0 };
    biquad_parameters_t pQ = { .type = BIQUAD_TYPE_NOTCH, .freq = 100.0, .q = sqrt(2.0) };
    biquad_coefficients_t cBW = make_coeffs(&pBW, 44100);
    biquad_coefficients_t cQ = make_coeffs(&pQ, 44100);
    ASSERT_TRUE(is_close_relative(cBW.a1, cQ.a1, 0.001));
    ASSERT_TRUE(is_close_relative(cBW.a2, cQ.a2, 0.001));
...
TEST(AllpassBWvsQ) {
    biquad_parameters_t pBW = { .type = BIQUAD_TYPE_ALLPASS, .freq = 100.0, .bandwidth = 1.0 };
    biquad_parameters_t pQ = { .type = BIQUAD_TYPE_ALLPASS, .freq = 100.0, .q = sqrt(2.0) };
    biquad_coefficients_t cBW = make_coeffs(&pBW, 44100);
    biquad_coefficients_t cQ = make_coeffs(&pQ, 44100);
    ASSERT_TRUE(is_close_relative(cBW.a1, cQ.a1, 0.001));
    ASSERT_TRUE(is_close_relative(cBW.a2, cQ.a2, 0.001));
...
TEST(HighshelfFO) {
    biquad_parameters_t params = { .type = BIQUAD_TYPE_HIGHSHELF_FO, .freq = 100.0, .gain = -12.0 };
    biquad_coefficients_t coeffs = make_coeffs(&params, 44100);
    double gf0, pf0, ghf, phf, glf, plf;
    gain_and_phase(&coeffs, 100.0, 44100.0, &gf0, &pf0);
    gain_and_phase(&coeffs, 10000.0, 44100.0, &ghf, &phf);
...
TEST(LowshelfFO) {
    biquad_parameters_t params = { .type = BIQUAD_TYPE_LOWSHELF_FO, .freq = 100.0, .gain = -12.0 };
    biquad_coefficients_t coeffs = make_coeffs(&params, 44100);
    double gf0, pf0, ghf, phf, glf, plf;
    gain_and_phase(&coeffs, 100.0, 44100.0, &gf0, &pf0);
    gain_and_phase(&coeffs, 10000.0, 44100.0, &ghf, &phf);
...
TEST(GeneralNotchHP) {
    biquad_parameters_t params = {
        .type = BIQUAD_TYPE_GENERAL_NOTCH,
        .q = 1.0, .freq_notch = 1000.0, .freq_pole = 2000.0, .normalize_at_dc = false
    };
    biquad_coefficients_t coeffs = make_coeffs(&params, 44100);
...
TEST(GeneralNotchLP) {
    biquad_parameters_t params = {
        .type = BIQUAD_TYPE_GENERAL_NOTCH,
        .q = 1.0, .freq_notch = 1000.0, .freq_pole = 500.0, .normalize_at_dc = true
    };
    biquad_coefficients_t coeffs = make_coeffs(&params, 44100);
    double gain_fp, p1, gain_hf, p2, gain_lf, p3;
    gain_and_phase(&coeffs, 1000.0, 44100.0, &gain_fp, &p1);
...
TEST(LinkwitzTransform) {
    biquad_parameters_t params = {
        .type = BIQUAD_TYPE_LINKWITZ_TRANSFORM,
        .freq_act = 100.0, .q_act = 1.2, .freq_target = 25.0, .q_target = 0.7
    };
    biquad_coefficients_t coeffs = make_coeffs(&params, 44100);
