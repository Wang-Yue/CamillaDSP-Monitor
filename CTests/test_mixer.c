...
static audio_chunk_t* make_constant_chunk(size_t frames, size_t channels, double value) {
...
static void assert_all_samples_ch(const audio_chunk_t* chunk, size_t ch, double expected, double accuracy) {
    waveform_t buf = audio_chunk_get_channel(chunk, ch);
    for (size_t i = 0; i < chunk->valid_frames; i++) {
        double diff = fabs(buf[i] - expected);
        if (diff > accuracy) {
            printf("Sample mismatch at ch %zu, idx %zu: expected %g, got %g (diff %g)\n", ch, i, expected, buf[i], diff);
        }
...
TEST(MixerConstruction2to4) {
    ...
    mixer_source_t src1 = { .channel = 1, .gain = 0.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    mixer_mapping_t maps[4] = {
        { .dest = 0, .sources_count = 1, .sources = &src0 },
...
TEST(MixerMutedSource) {
    ...
    for (int i = 0; i < 4; i++) { b0[i] = 1.0; b1[i] = 0.5; }
...
TEST(MixerStereoToMono) {
    mixer_source_t srcs[2] = {
        { .channel = 0, .gain = -6.0, .has_gain = true },
...
TEST(MixerMonoToStereo) {
    ...
    mixer_config_t config = { .channels_in = 1, .channels_out = 2, .mapping_count = 2, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(Mixer4to2Downmix) {
    ...
    mixer_config_t config = { .channels_in = 4, .channels_out = 2, .mapping_count = 2, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(MixerWithInvertedSource) {
    ...
    mixer_mapping_t map = { .dest = 0, .sources_count = 2, .sources = srcs };
    mixer_config_t config = { .channels_in = 1, .channels_out = 1, .mapping_count = 1, .mapping = &map };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(MixerIdentity) {
    ...
    mixer_config_t config = { .channels_in = 2, .channels_out = 2, .mapping_count = 2, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
    ...
    mutable_waveform_t b0 = audio_chunk_get_channel(input, 0);
    mutable_waveform_t b1 = audio_chunk_get_channel(input, 1);
    for (int i = 0; i < 4; i++) { b0[i] = ch0[i]; b1[i] = ch1[i]; }
...
TEST(MixerChannelRouting) {
    ...
    mixer_source_t src0 = { .channel = 0, .gain = 0.0, .has_gain = true };
    mixer_mapping_t maps[2] = {
        { .dest = 0, .sources_count = 1, .sources = &src1 },
        { .dest = 1, .sources_count = 1, .sources = &src0 }
    };
    mixer_config_t config = { .channels_in = 2, .channels_out = 2, .mapping_count = 2, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(MixerGainAccuracy) {
    ...
    mixer_source_t src2 = { .channel = 0, .gain = 0.0, .has_gain = true, .mute = true };
    mixer_mapping_t maps[3] = {
        { .dest = 0, .sources_count = 1, .sources = &src0 },
        { .dest = 1, .sources_count = 1, .sources = &src1 },
        { .dest = 2, .sources_count = 1, .sources = &src2 }
    };
    mixer_config_t config = { .channels_in = 1, .channels_out = 3, .mapping_count = 3, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(MixerWithLinearScale) {
    mixer_source_t src0_0 = { .channel = 0, .gain = 0.5, .has_gain = true, .scale = GAIN_SCALE_LINEAR };
    mixer_source_t src0_1 = { .channel = 0, .gain = 2.0, .has_gain = true, .scale = GAIN_SCALE_LINEAR };
    mixer_source_t srcs2[2] = {
        { .channel = 0, .gain = 0.5, .has_gain = true, .scale = GAIN_SCALE_LINEAR },
        { .channel = 1, .gain = 0.5, .has_gain = true, .scale = GAIN_SCALE_LINEAR }
    };
    mixer_mapping_t maps[3] = {
    ...
    mixer_config_t config = { .channels_in = 2, .channels_out = 3, .mapping_count = 3, .mapping = maps };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(CheckMakeMixer) {
    ...
    mutable_waveform_t b0 = audio_chunk_get_channel(input, 0);
    mutable_waveform_t b1 = audio_chunk_get_channel(input, 1);
    b0[0] = 1.0; b0[1] = 0.0; b0[2] = 0.0; b0[3] = 0.0;
    b1[0] = 0.0; b1[1] = 1.0; b1[2] = 0.0; b1[3] = 0.0;
    ...
    ASSERT_NEAR(1.0, o0[0], 1e-9); ASSERT_NEAR(0.0, o0[1], 1e-9);
    ASSERT_NEAR(0.0, o1[0], 1e-9); ASSERT_NEAR(1.0, o1[1], 1e-9);
    ASSERT_NEAR(1.0, o2[0], 1e-9); ASSERT_NEAR(0.0, o2[1], 1e-9);
    ASSERT_NEAR(0.0, o3[0], 1e-9); ASSERT_NEAR(1.0, o3[1], 1e-9);
...
TEST(MixerUnmappedOutputIsSilent) {
    mixer_source_t src0 = { .channel = 0, .gain = 0.0, .has_gain = true };
    mixer_mapping_t map = { .dest = 0, .sources_count = 1, .sources = &src0 };
    mixer_config_t config = { .channels_in = 1, .channels_out = 2, .mapping_count = 1, .mapping = &map };
    audio_mixer_t* mixer = audio_mixer_create("mixer", &config, 2048);
...
TEST(MixerInoutAPI_MatchesAllocatingAPI) {
    mixer_source_t srcs0[2] = {
        { .channel = 0, .gain = -3.0, .has_gain = true },
        { .channel = 1, .gain = -6.0, .has_gain = true }
    };
    mixer_source_t src1 = { .channel = 1, .gain = 0.0, .has_gain = true };
    mixer_source_t src2 = { .channel = 0, .gain = 0.0, .has_gain = true, .mute = true };
...
TEST(MixerInoutAPI_OverwritesPriorData) {
    ...
    audio_chunk_t* input = audio_chunk_create(4, 1);
    mutable_waveform_t b0 = audio_chunk_get_channel(input, 0);
    b0[0] = 1.0; b0[1] = 2.0; b0[2] = 3.0; b0[3] = 4.0;
