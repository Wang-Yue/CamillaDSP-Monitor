...
#include "../CLib/FFT/bluestein_fft.h"
#include "../CLib/FFT/real_fft.h"
#include <math.h>
#include <stdlib.h>
#include <stdint.h>
...
typedef struct { uint64_t state; } simple_splitmix_t;
...
TEST(ForwardMatchesDirectDFT) {
    ...
    for (size_t i = 0; i < sizeof(sizes)/sizeof(sizes[0]); i++) {
        ...
        free(in_re); free(in_im); free(bs_re); free(bs_im); free(dir_re); free(dir_im);
    }
}
...
TEST(RealFFTFallbackForPrimeFactors) {
    ...
    free(input); free(spec_re); free(spec_im); free(recovered);
}
