...
#include "../CLib/Resampler/audio_resampler.h"
...
static const size_t g_rate_grid_count = sizeof(g_rate_grid) / sizeof(g_rate_grid[0]);
...
static const size_t g_swift_bounds_count = sizeof(g_swift_bounds) / sizeof(g_swift_bounds[0]);
...
static bool check_rubato_available(void) {
    ...
    const char* candidates[] = {
        ...
        "../Tests/RustHarnesses/target/release/cdsp_resampler_compare",
        "../../Tests/RustHarnesses/target/release/cdsp_resampler_compare",
        "/Users/wangyue/CamillaDSP-Monitor/Tests/RustHarnesses/target/release/cdsp_resampler_compare"
    };
    ...
    printf("⚠️ skipping: rubato harness not found — build with `make -C Tests/RustHarnesses`\n");
...
static double impulse_asymmetry(const double* output, size_t count, int window) {
...
static double* run_resampler_full(audio_resampler_t* res, const double* input, size_t input_count, size_t* out_count) {
    ...
    while (idx + cs <= input_count) {
        ...
        if (audio_resampler_process(res, in_chunk, out_chunk) == RESAMPLER_OK) {
            ...
            if (total_out + produced > estimated_out) {
                ...
                double* new_out = (double*)realloc(output, estimated_out * sizeof(double));
                if (!new_out) {
                    free(output);
...
static double* run_rubato(const char* mode, int in_rate, int out_rate, const double* input, size_t input_count, size_t* out_count) {
    char in_path[256];
    char out_path[256];
    snprintf(in_path, sizeof(in_path), "/tmp/cdsp_matrix_%s_%d_%d_%zu_in.raw", mode, in_rate, out_rate, input_count);
    snprintf(out_path, sizeof(out_path), "/tmp/cdsp_matrix_%s_%d_%d_%zu_out.raw", mode, in_rate, out_rate, input_count);
    
    if (!write_raw_f64(input, input_count, in_path)) return NULL;
    
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "\"%s\" %s \"%s\" \"%s\" %d %d %d --no-partial >/dev/null 2>&1",
...
static double* run_process(int impl_id, const double* input, size_t input_count, int in_rate, int out_rate, size_t* out_count) {
    ...
    switch (impl_id) {
        ...
        case 6:
            if (!check_rubato_available()) return NULL;
            return run_rubato("poly-septic", in_rate, out_rate, input, input_count, out_count);
        case 7:
            if (!check_rubato_available()) return NULL;
            return run_rubato("sinc-accurate", in_rate, out_rate, input, input_count, out_count);
        default:
            return NULL;
    }

    size_t cs = 1024;
    audio_resampler_t* res = audio_resampler_create_from_config(&cfg, in_rate, out_rate, 1, cs);
    if (!res) return NULL;
...
static cell_t measure_quality_cell(int in_rate, int out_rate, int impl_id) {
    ...
    if (out_rate < in_rate) {
        ...
        size_t out_count = 0;
        double* out = run_process(impl_id, signal, nbr_in, in_rate, out_rate, &out_count);
    ...
    if (imp_signal) {
        ...
        double* imp_out = run_process(impl_id, imp_signal, imp_count, in_rate, out_rate, &imp_out_count);
    ...
    if (sin_signal) {
        size_t up_count = 0;
        double* up_out = run_process(impl_id, sin_signal, nbr_in, in_rate, out_rate, &up_count);
        if (up_out && up_count > 0) {
            size_t down_count = 0;
            double* down_out = run_process(impl_id, up_out, up_count, out_rate, in_rate, &down_count);
            if (down_out && down_count > 0) {
                size_t skip = 4 * cs;
                size_t end_idx = skip + 8192 < down_count ? skip + 8192 : down_count;
                if (end_idx > skip) {
                    double sinad = sinad_db(down_out + skip, end_idx - skip, in_rate, 1000.0);
                    if (!isnan(sinad)) {
                        c.sinad_db = sinad;
                        c.has_sinad_db = true;
                    }
...
static bool measure_swift_perf(int in_rate, int out_rate, int impl_id, double* out_ns_per_frame, double* out_rtf) {
    resampler_config_t cfg;
    switch (impl_id) {
        case 0: resampler_config_init(&cfg, RESAMPLER_TYPE_SYNCHRONOUS); break;
        case 1: 
            resampler_config_init(&cfg, RESAMPLER_TYPE_ASYNC_POLY);
            strncpy(cfg.interpolation, "Septic", sizeof(cfg.interpolation) - 1);
            cfg.has_interpolation = true;
    ...
    size_t cs = 1024;
    audio_resampler_t* resampler = audio_resampler_create_from_config(&cfg, in_rate, out_rate, 1, cs);
    ...
    while (true) {
        ...
        for (size_t c = 0; c < chunk_count; c++) {
            if (audio_resampler_process(resampler, chunks[c], scratch) == RESAMPLER_OK) {
...
static bool measure_rubato_perf(const char* mode, int in_rate, int out_rate, double* out_ns_per_frame, double* out_rtf) {
    ...
    char in_path[256];
    char out_path[256];
    snprintf(in_path, sizeof(in_path), "/tmp/cdsp_matrix_perf_%s_%d_%d_in.raw", mode, in_rate, out_rate);
    snprintf(out_path, sizeof(out_path), "/tmp/cdsp_matrix_perf_%s_%d_%d_out.raw", mode, in_rate, out_rate);
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "\"%s\" %s \"%s\" \"%s\" %d %d %zu --bench=20 2>&1",
...
static row_t compute_row_for_rate_pair(int index, int in_rate, int out_rate, const char* label, bool rubato_ok) {
    row_t r;
    memset(&r, 0, sizeof(r));
    r.index = index;
    r.label = label;
    ...
    if (measure_swift_perf(in_rate, out_rate, 0, &ns, &rtf)) {
        r.swift_sync.ns_per_out_frame = ns; r.swift_sync.has_ns_per_out_frame = true;
        r.swift_sync.rtf_per_iter = rtf; r.swift_sync.has_rtf_per_iter = true;
    }
    if (measure_swift_perf(in_rate, out_rate, 1, &ns, &rtf)) {
        r.swift_poly.ns_per_out_frame = ns; r.swift_poly.has_ns_per_out_frame = true;
        r.swift_poly.rtf_per_iter = rtf; r.swift_poly.has_rtf_per_iter = true;
    }
    if (measure_swift_perf(in_rate, out_rate, 2, &ns, &rtf)) {
        r.swift_sinc.ns_per_out_frame = ns; r.swift_sinc.has_ns_per_out_frame = true;
        r.swift_sinc.rtf_per_iter = rtf; r.swift_sinc.has_rtf_per_iter = true;
    }
    if (measure_swift_perf(in_rate, out_rate, 3, &ns, &rtf)) {
        r.apple_mast.ns_per_out_frame = ns; r.apple_mast.has_ns_per_out_frame = true;
        r.apple_mast.rtf_per_iter = rtf; r.apple_mast.has_rtf_per_iter = true;
    }
    if (measure_swift_perf(in_rate, out_rate, 4, &ns, &rtf)) {
        r.apple_minph.ns_per_out_frame = ns; r.apple_minph.has_ns_per_out_frame = true;
        r.apple_minph.rtf_per_iter = rtf; r.apple_minph.has_rtf_per_iter = true;
    }

    if (rubato_ok && r.rubato_fft.has_sinad_db) {
        if (measure_rubato_perf("fft", in_rate, out_rate, &ns, &rtf)) {
            r.rubato_fft.ns_per_out_frame = ns; r.rubato_fft.has_ns_per_out_frame = true;
            r.rubato_fft.rtf_per_iter = rtf; r.rubato_fft.has_rtf_per_iter = true;
        }
    }
    if (rubato_ok && r.rubato_poly.has_sinad_db) {
        if (measure_rubato_perf("poly-septic", in_rate, out_rate, &ns, &rtf)) {
            r.rubato_poly.ns_per_out_frame = ns; r.rubato_poly.has_ns_per_out_frame = true;
            r.rubato_poly.rtf_per_iter = rtf; r.rubato_poly.has_rtf_per_iter = true;
        }
    }
    if (rubato_ok && r.rubato_sinc.has_sinad_db) {
        if (measure_rubato_perf("sinc-accurate", in_rate, out_rate, &ns, &rtf)) {
            r.rubato_sinc.ns_per_out_frame = ns; r.rubato_sinc.has_ns_per_out_frame = true;
            r.rubato_sinc.rtf_per_iter = rtf; r.rubato_sinc.has_rtf_per_iter = true;
        }
    }
...
static bool get_cell_metric(const cell_t* c, metric_type_t metric, double* out_val) {
    switch (metric) {
        case METRIC_SINAD:
            if (c->has_sinad_db && isfinite(c->sinad_db)) { *out_val = c->sinad_db; return true; }
            return false;
        case METRIC_ALIASING:
            if (c->has_aliasing_db && isfinite(c->aliasing_db)) { *out_val = c->aliasing_db; return true; }
            return false;
        case METRIC_PASSBAND:
            if (c->has_passband_db && isfinite(c->passband_db)) { *out_val = c->passband_db; return true; }
            return false;
        case METRIC_SYMMETRY:
            if (c->has_symmetry && isfinite(c->symmetry)) { *out_val = c->symmetry; return true; }
            return false;
        case METRIC_NS_PER_FRAME:
            if (c->has_ns_per_out_frame && isfinite(c->ns_per_out_frame)) { *out_val = c->ns_per_out_frame; return true; }
            return false;
        case METRIC_RTF:
            if (c->has_rtf_per_iter && isfinite(c->rtf_per_iter)) { *out_val = c->rtf_per_iter; return true; }
            return false;
    }
    return false;
}

static void format_cell(bool has_val, double val, const char* format_str, bool has_best, double best_val, char* out_14) {
...
static void print_table(const row_t* grid, size_t grid_count, const char* title, metric_type_t metric, bool higher_is_better, const char* format_str) {
    ...
    const char* direction_str = higher_is_better ? "higher is better" : "lower is better";
    printf("=== %s (%s) ===\n", title, direction_str);
    printf("%s %s %s %s %s %s %s %s %s\n", pair_col, h0, h1, h2, h3, h4, h5, h6, h7);

    for (size_t r = 0; r < grid_count; r++) {
        const row_t* row = &grid[r];
        const cell_t* cells[8] = {
            &row->swift_sync, &row->swift_poly, &row->swift_sinc,
            &row->apple_mast, &row->apple_minph,
            &row->rubato_fft, &row->rubato_poly, &row->rubato_sinc
        };
        double vals[8];
        bool has_vals[8];
...
TEST(compareAcrossRateGrid) {
    ...
    for (size_t i = 0; i < g_rate_grid_count; i++) {
        grid[i] = compute_row_for_rate_pair(
            (int)i, g_rate_grid[i].in_rate, g_rate_grid[i].out_rate, g_rate_grid[i].label, rubato_ok
        );
    }

    print_table(grid, g_rate_grid_count, "Round-trip SINAD (1 kHz sine)", METRIC_SINAD, true, "%6.1f dB");
    print_table(grid, g_rate_grid_count, "Aliasing rejection (mid-stopband tone)", METRIC_ALIASING, false, "%7.1f dB");
    print_table(grid, g_rate_grid_count, "Passband peak deviation", METRIC_PASSBAND, false, "%7.4f dB");
    print_table(grid, g_rate_grid_count, "Impulse-response asymmetry (rel to peak)", METRIC_SYMMETRY, false, "%9.3e");
    print_table(grid, g_rate_grid_count, "Throughput (ns/output-frame)", METRIC_NS_PER_FRAME, false, "%8.1f");
    print_table(grid, g_rate_grid_count, "Throughput (real-time factor per iteration)", METRIC_RTF, true, "%7.1fx");
