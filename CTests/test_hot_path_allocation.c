...
#include "../CLib/Resampler/apple_resampler.h"
#include "../CLib/Resampler/synchronous_resampler.h"
#include "../CLib/Resampler/async_poly_resampler.h"
#include "../CLib/Resampler/async_sinc_resampler.h"
#include "../CLib/Filters/biquad.h"
#include "../CLib/Filters/convolution.h"
#include "../CLib/Filters/gain.h"
#include "../CLib/Filters/volume.h"
#include "../CLib/Filters/loudness.h"
#include "../CLib/Filters/delay.h"
#include "../CLib/Filters/biquad_combo.h"
#include "../CLib/Filters/diffeq.h"
#include "../CLib/Filters/dither.h"
#include "../CLib/Filters/limiter.h"
#include "../CLib/Filters/lookahead_limiter.h"
#include "../CLib/Processors/compressor_processor.h"
#include "../CLib/Processors/noise_gate_processor.h"
#include "../CLib/Processors/race_processor.h"
#include "../CLib/Mixer/mixer.h"
#include "../CLib/DoP/dop_encoder.h"
#include "../CLib/DoP/dop_decoder.h"
#include "../CLib/Logging/app_logger.h"
#include "../CLib/Audio/processing_parameters.h"
#include "../CLib/Audio/audio_buffers.h"
...
static void my_malloc_logger(uint32_t type, uintptr_t arg1, uintptr_t arg2, uintptr_t arg3, uintptr_t result, uint32_t num_hot_frames_to_skip) {
    (void)arg1; (void)arg2; (void)arg3; (void)num_hot_frames_to_skip;
    if ((type & 2) != 0 && result != 0) {
        uintptr_t watched = atomic_load_explicit(&g_watched_thread, memory_order_acquire);
...
static bool count_allocations(void (*body)(void*), void* ctx, uint64_t* out_count) {
    void* handle = dlopen(NULL, RTLD_LAZY);
    if (!handle) return false;
    malloc_logger_t* logger_ptr = (malloc_logger_t*)dlsym(handle, "malloc_logger");
    if (!logger_ptr) {
        dlclose(handle);
        return false;
    }
...
static void assert_allocation_free(const char* label, int warmup, int iterations, test_iter_func_t body, void* ctx) {
    ...
    printf("[%s] allocations=%llu over %d iterations\n", label, (unsigned long long)count, iterations);
    ASSERT_TRUE(count < 10);
}

static audio_chunk_t** make_random_chunks(int count, int channels, int frames, double scale) {
    audio_chunk_t** chunks = (audio_chunk_t**)calloc(count, sizeof(audio_chunk_t*));
...
static void fill_sine(mutable_waveform_t buf, int frames, double freq_hz, double sample_rate) {
...
static void resampler_iter(int i, void* ctx) {
    ...
    audio_resampler_process(c->resampler, c->inputs[i % c->input_count], c->output);
}

static void run_resampler_hot_path(audio_resampler_t* resampler, int channels, const char* label) {
...
TEST(AppleResampler_AllocationFree_Stereo) {
    ...
    audio_resampler_t* res = audio_resampler_create_from_config(&cfg, 44100, 48000, 2, 1024);
...
TEST(Biquad_AllocationFree) {
    biquad_parameters_t params = { .type = BIQUAD_TYPE_LOWPASS, .freq = 1000.0, .q = 0.707 };
    biquad_coefficients_t coeffs = { .b0 = 0.25, .b1 = 0.5, .b2 = 0.25, .a1 = -0.5, .a2 = 0.1 };
    (void)params;
...
TEST(Volume_AllocationFree) {
    ...
    processing_parameters_set_target_volume_for_fader(proc_params, -6.0, FADER_MAIN);
...
TEST(Loudness_AllocationFree) {
    processing_parameters_t* proc_params = processing_parameters_create(2, 2);
    processing_parameters_set_current_volume_for_fader(proc_params, -45.0, FADER_MAIN);
...
TEST(BiquadCombo_AllocationFree) {
    biquad_combo_parameters_t params = {
        .type = BIQUAD_COMBO_TYPE_FIVE_POINT_PEQ,
...
static void comp_proc_wrap(void* p, audio_chunk_t* c) { compressor_processor_process((compressor_processor_t*)p, c); }
static void gate_proc_wrap(void* p, audio_chunk_t* c) { noise_gate_processor_process((noise_gate_processor_t*)p, c); }
static void race_proc_wrap(void* p, audio_chunk_t* c) { race_processor_process((race_processor_t*)p, c); }
...
TEST(Compressor_AllocationFree) {
    int mon_ch[] = {0};
    int proc_ch[] = {0, 1};
    compressor_parameters_t params = { .channels = 2, .monitor_channels = mon_ch, .monitor_channels_count = 1, .process_channels = proc_ch, .process_channels_count = 2, .attack = 0.005, .release = 0.05, .threshold = -10.0, .factor = 3.0, .makeup_gain = 2.0, .has_makeup_gain = true, .soft_clip = true, .clip_limit = -1.0, .has_clip_limit = true };
    compressor_processor_t* proc = compressor_processor_create("comp", &params, 44100, 1024);
...
TEST(NoiseGate_AllocationFree) {
    int mon_ch[] = {0};
    int proc_ch[] = {0, 1};
    noise_gate_parameters_t params = { .channels = 2, .monitor_channels = mon_ch, .monitor_channels_count = 1, .process_channels = proc_ch, .process_channels_count = 2, .attack = 0.005, .release = 0.05, .threshold = -20.0, .attenuation = 12.0 };
    noise_gate_processor_t* proc = noise_gate_processor_create("gate", &params, 44100, 1024);
    ASSERT_TRUE(proc != NULL);
    audio_chunk_t* chunk = audio_chunk_create(1024, 2);
    for (size_t f = 0; f < 1024; f++) {
        audio_chunk_get_channel(chunk, 0)[f] = 0.5;
...
TEST(RACE_AllocationFree) {
    race_parameters_t params = { .channels = 2, .channel_a = 0, .channel_b = 1, .delay = 12.0, .subsample_delay = false, .has_subsample_delay = true, .delay_unit = DELAY_UNIT_SAMPLES, .has_delay_unit = true, .attenuation = 6.0 };
...
TEST(Mixer_2to4_AllocationFree) {
    mixer_source_t s00 = { .channel = 0, .gain = 0.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    mixer_source_t s11 = { .channel = 1, .gain = 0.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    mixer_source_t s20 = { .channel = 0, .gain = -3.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    mixer_source_t s21 = { .channel = 1, .gain = -3.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    mixer_source_t s2_srcs[] = { s20, s21 };
    mixer_source_t s31 = { .channel = 1, .gain = -6.0, .has_gain = true, .scale = GAIN_SCALE_DB };
    ...
    mixer_config_t config = { .channels_in = 2, .channels_out = 4, .mapping_count = 4, .mapping = maps };
...
TEST(DoPEncoder_AllocationFree) {
    dop_encoder_t* encoder = dop_encoder_create(2, 176400.0, true, SDM_FILTER_SDM4, 20000.0);
...
TEST(DoPDecoder_AllocationFree) {
    ...
    int total_chunks = 36;
    audio_chunk_t** chunks = (audio_chunk_t**)calloc(total_chunks, sizeof(audio_chunk_t*));
...
static void logger_iter(int i, void* ctx) {
    ...
    logger_info(&logger, "Test event: int=%d, float=%f, static=%s", log_arg_int((int64_t)i), log_arg_double(3.14159 + (double)i), log_arg_string("Static string argument value"), log_arg_none());
}
...
static void proc_params_iter(int i, void* ctx) {
    ...
    processing_parameters_update_capture_levels(c->params, c->chunks[i % c->chunk_count]);
    processing_parameters_update_playback_levels(c->params, c->chunks[i % c->chunk_count]);
}
...
TEST(ProcessingParameters_AllocationFree) {
    ...
    assert_allocation_free("ProcessingParameters updateLevels", 0, 30, proc_params_iter, &ctx);
