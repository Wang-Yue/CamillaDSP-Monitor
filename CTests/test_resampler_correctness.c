...
static void assert_stereo_matches_mono(resampler_type_t type, const char* interp_str) {
    int in_rate = 44100;
    int out_rate = 48000;
    ...
    if (interp_str) {
        strncpy(cfg_stereo.interpolation, interp_str, sizeof(cfg_stereo.interpolation) - 1);
    ...
    audio_resampler_t* stereo = audio_resampler_create_from_config(&cfg_stereo, in_rate, out_rate, 2, chunk_size);
    audio_resampler_t* mono_l = audio_resampler_create_from_config(&cfg_stereo, in_rate, out_rate, 1, chunk_size);
    audio_resampler_t* mono_r = audio_resampler_create_from_config(&cfg_stereo, in_rate, out_rate, 1, chunk_size);
    ...
    audio_chunk_free(st_in); audio_chunk_free(st_out);
    audio_chunk_free(ml_in); audio_chunk_free(ml_out);
    audio_chunk_free(mr_in); audio_chunk_free(mr_out);
...
static void assert_inout_matches(resampler_type_t type, const char* interp_str) {
    ...
    size_t chunk_size = 1024;
    audio_resampler_t* res_a = audio_resampler_create_from_config(&cfg, 44100, 48000, 2, chunk_size);
    audio_resampler_t* res_b = audio_resampler_create_from_config(&cfg, 44100, 48000, 2, chunk_size);
...
static void assert_rejects_too_small(resampler_type_t type, const char* interp_str) {
    resampler_config_t cfg;
    resampler_config_init(&cfg, type);
    if (interp_str) {
        strncpy(cfg.interpolation, interp_str, sizeof(cfg.interpolation) - 1);
    ...
    size_t chunk_size = 1024;
    audio_resampler_t* res = audio_resampler_create_from_config(&cfg, 44100, 48000, 2, chunk_size);
