...
/// Errors raised by `AudioHistoryBuffer.readLatest`. The buffer is
/// general-purpose (used by spectrum analysis *and* `getSamples`), so
/// it owns its own error type rather than borrowing from the spectrum
/// analyzer.
internal enum AudioHistoryBufferError: Error, Sendable, CustomStringConvertible {
  ...
  internal var description: String {
    switch self {
    case .bufferEmpty:
      return "Audio history buffer is empty"
    case .channelOutOfRange(let ch, let avail):
      return "Channel \(ch) out of range (available: \(avail))"
    }
  }
}
...
public final class AudioHistoryBuffer: @unchecked Sendable {
  ...
  public func readLatest(
    ...
    guard channels > 0 else { throw AudioHistoryBufferError.bufferEmpty }
    if let ch = channel {
