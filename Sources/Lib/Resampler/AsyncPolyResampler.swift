...
public final class AsyncPolyResampler: AudioResampler {
  ...
  // no Array CoW on the audio thread.
  ...
  private func runCubicLoop(
    idxBase: UnsafePointer<Int>,
    fracBase: UnsafePointer<Double>,
    outputFrames: Int,
    baseOffset: Int,
    output: inout AudioChunk
  ) {
    ...
    for ch in 0..<channels {
      ...
      while frame < pairCount4 {
        // Pair 1
        let baseA = buf + idxBase[frame] + baseOffset
        let baseB = buf + idxBase[frame + 1] + baseOffset
        ...
        // Pair 2
        let baseC = buf + idxBase[frame + 2] + baseOffset
        let baseD = buf + idxBase[frame + 3] + baseOffset
      ...
      if frame < outputFrames {
        let x = fracBase[frame]
        let base = buf + idxBase[frame] + baseOffset
        let y0 = base[0]
        let y1 = base[1]
        let y2 = base[2]
        let y3 = base[3]
