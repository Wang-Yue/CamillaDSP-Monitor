...
/// Stage detail view for the Convolution stage. Mirrors `EQOptions`
/// — same channel-mode picker, same per-channel preset picker — so
/// the user gets identical UX to the EQ stage they're already
/// familiar with.
struct ConvolutionOptions: View {
      ...
      GroupBox("Channel Mode") {
        HStack(spacing: 16) {
          Text("Mode")
            ...
            .foregroundStyle(.secondary)
            .fixedSize()
          Picker("", selection: $stage.convChannelMode) {
            ForEach(ConvChannelMode.allCases) { mode in Text(mode.rawValue).tag(mode) }
          }
          .pickerStyle(.segmented)
          .labelsHidden()
          .frame(maxWidth: 400)
          .onChange(of: stage.convChannelMode) { _, _ in dsp.applyConfig() }
              ...
              VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 16) {
                  Text("Preset")
                    ...
                    .fixedSize()
                  ConvPresetPicker(
                    selectedID: $stage.convLeftPresetID, presets: pipeline.convPresets
                  )
                  .frame(maxWidth: 400)
                  .onChange(of: stage.convLeftPresetID) { _, _ in dsp.applyConfig() }
                  Spacer()
                }

                if let preset = pipeline.convPresets.first(where: {
                  $0.id == stage.convLeftPresetID
                }) {
                  ConvolutionSummaryCard(preset: preset, sampleRate: liveRate)
                }
              }
              ...
              VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 16) {
                  Text("Preset")
                    ...
                    .fixedSize()
                  ConvPresetPicker(
                    selectedID: $stage.convRightPresetID, presets: pipeline.convPresets
                  )
                  .frame(maxWidth: 400)
                  .onChange(of: stage.convRightPresetID) { _, _ in dsp.applyConfig() }
                  Spacer()
                }

                if let preset = pipeline.convPresets.first(where: {
                  $0.id == stage.convRightPresetID
                }) {
                  ConvolutionSummaryCard(preset: preset, sampleRate: liveRate)
                }
              }
  ...
  /// Sample rate the engine is currently driving — that's the IR
  /// member the live filter graph would actually use, so it's the
  /// right one to surface in the summary card.
  private var liveRate: Int { devices.captureConfig.sampleRate }
}

/// Compact "what does this preset look like at the live rate" card.
/// Mirrors `EQSummaryCard` in shape: a metadata strip + an inline
/// IR plot, sized small enough to fit beside the picker.
                ...
                Picker("", selection: $settings.resamplerType) {
                  ...
                  ForEach(
                    ResamplerType.allCases.filter { type in
                      CamillaDSPEngine.isSwiftEngine
