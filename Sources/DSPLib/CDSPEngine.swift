...
extension Fader {
  var cValue: fader_t {
    switch self {
    case .main: return FADER_MAIN
    case .aux1: return FADER_AUX1
    case .aux2: return FADER_AUX2
    case .aux3: return FADER_AUX3
    case .aux4: return FADER_AUX4
    }
  }
}
...
public actor DSPEngine {
  ...
  public func start(configJson: String) async throws {
    ...
    var err = audio_backend_error_t()
    let success = configJson.withCString { cStr in
      dsp_engine_set_config(e, cStr, &err)
    }
    if !success {
      let msg = withUnsafePointer(to: &err.message) { ptr in
        ptr.withMemoryRebound(to: CChar.self, capacity: 256) { String(cString: $0) }
      }
      if err.type == AUDIO_BACKEND_ERR_COMMAND_SEND {
        throw AudioBackendError.commandSend(message: msg)
      } else if err.type == AUDIO_BACKEND_ERR_INVALID_SAMPLERATE {
        throw AudioBackendError.invalidSamplerate(message: msg)
      } else {
  ...
  public func setFaderMute(_ fader: Fader, _ mute: Bool) async {
    guard let e = engine else { return }
    dsp_engine_set_fader_mute(e, fader.cValue, mute)
  }

  public func getFaderVolume(_ fader: Fader) async -> Float {
    guard let e = engine else { return 0.0 }
    return dsp_engine_get_fader_volume(e, fader.cValue)
  }

  public func isFaderMuted(_ fader: Fader) async -> Bool {
    guard let e = engine else { return false }
    return dsp_engine_is_fader_muted(e, fader.cValue)
  }

  public func getStatus() async -> StateUpdate {
    guard let e = engine else { return StateUpdate(state: .inactive, stopReason: .none) }
    let st = dsp_engine_get_status(e)
    let state: ProcessingState
    switch st.state {
    case PROCESSING_STATE_RUNNING: state = .running
    case PROCESSING_STATE_PAUSED: state = .paused
    case PROCESSING_STATE_INACTIVE: state = .inactive
    case PROCESSING_STATE_STARTING: state = .starting
    case PROCESSING_STATE_STALLED: state = .stalled
    default: state = .inactive
    }
    let stopReason: ProcessingStopReason
    switch st.stop_reason.type {
    case STOP_REASON_NONE: stopReason = .none
    case STOP_REASON_DONE: stopReason = .done
    case STOP_REASON_CAPTURE_ERROR:
      let msg = withUnsafePointer(to: st.stop_reason.message) { ptr in
        ptr.withMemoryRebound(to: CChar.self, capacity: 256) { String(cString: $0) }
      }
    ...
    case STOP_REASON_CAPTURE_FORMAT_CHANGE:
      stopReason = .captureFormatChange(Int(st.stop_reason.format_change_rate))
    case STOP_REASON_PLAYBACK_FORMAT_CHANGE:
      stopReason = .playbackFormatChange(Int(st.stop_reason.format_change_rate))
  ...
  public func getSpectrum(
    ...
    let ch = channel != nil ? Int32(channel!) : -1
    let st = dsp_engine_get_spectrum(e, isCapture, ch, minFreq, maxFreq, Int(nBins), &res)
    if st != SPECTRUM_OK {
      if st == SPECTRUM_ERROR_EMPTY {
        throw AudioBackendError.bufferEmpty
      } else {
        throw AudioBackendError.spectrumCompute(message: "Spectrum compute error (\(st))")
      }
    }
    let freqs = Array(UnsafeBufferPointer(start: res.frequencies, count: Int(res.count)))
    let mags = Array(UnsafeBufferPointer(start: res.magnitudes, count: Int(res.count)))
  ...
  public func getSamples(isCapture: Bool, nFrames: UInt32) async throws -> AudioSamples {
    ...
    var err = audio_backend_error_t()
    guard let res = dsp_engine_get_samples(e, isCapture, Int(nFrames), &err) else {
      if err.type == AUDIO_BACKEND_ERR_ENGINE_NOT_RUNNING {
        throw AudioBackendError.engineNotRunning
      } else {
        throw AudioBackendError.bufferEmpty
      }
    }
    defer { dsp_engine_free_samples(res) }
    var channels: [[Float]] = []
  ...
  public func getAvailableDevices(backend: String, input: Bool) async -> [AudioDevice] {
    ...
    let count = backend.withCString { cStr in
      devs.withUnsafeMutableBufferPointer { ptr in
        dsp_engine_get_available_devices(cStr, input, ptr.baseAddress, 32)
      }
    }
    var res: [AudioDevice] = []
    for i in 0..<Int(count) {
      let name = withUnsafePointer(to: devs[i].name) { ptr in
        ptr.withMemoryRebound(to: CChar.self, capacity: 256) { String(cString: $0) }
      }
  ...
  public func setLogLevel(_ level: LogLevel) async {
    level.rawValue.withCString { cStr in
      dsp_engine_set_log_level(log_level_from_string(cStr))
    }
  }
}
