...
public final class CoreAudioCapture: CaptureBackend {
  ...
  var preallocBufferListByteCount: Int = 0
  ...
  public init(config: CaptureDeviceConfig, sampleRate: Int, chunkSize: Int) {
    ...
    self.ringBufferSize = chunkSize * 4
  ...
  public func open() throws {
    ...
    if let deviceID = resolvedDeviceID {
      if !CoreAudioDevice.setNominalSampleRate(deviceID, sampleRate) {
        logger.warning(
          "Capture device refused \(sampleRate) Hz; AudioUnit will sample-rate convert")
      }
    ...
    logger.info(
      "Device native format: \(deviceFormat.mSampleRate)Hz, \(deviceFormat.mChannelsPerFrame)ch, \(deviceFormat.mBitsPerChannel)bit, flags=\(deviceFormat.mFormatFlags)"
    )
  ...
  public func read(frames: Int, into chunk: inout AudioChunk) throws -> Bool {
    ...
    for ch in 0..<channels {
      ...
      chunk.waveforms[ch].withUnsafeMutableBufferPointer { dst in
  ...
  private func allocateRenderBuffers() {
    ...
    let bytesPerBuffer =
      isInterleaved
      ? chunkSize * channels * MemoryLayout<Float>.size
    ...
    preallocBufferListByteCount = listByteCount
...
private func captureCallback(
  ...
  guard status == noErr else {
    // Avoid `print` in steady state — it can take a Foundation lock
    // — but the first few errors are useful for diagnosing setup
    // issues.
  ...
  let frames = Swift.min(Swift.min(actualFrames, frameCount), capture.chunkSize)
