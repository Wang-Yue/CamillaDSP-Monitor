 ...
 *
 * This module defines a polymorphic C interface (`dsp_processor_t`) that mirrors the Swift
 * `Processor` protocol for all multi-channel audio processors (Compressor, Noise Gate, RACE).
 * It provides uniform dispatch for real-time in-place processing and dynamic parameter updates.
 */
...
typedef struct dsp_processor {
    ...
    void (*update_parameters)(struct dsp_processor* self, const processor_config_t* config, int sample_rate);
...
/**
 * @brief Factory function to create a processor instance based on configuration.
 *
 ...
 * @return Pointer to newly allocated dsp_processor_t wrapper, or NULL on failure.
 */
dsp_processor_t* dsp_processor_create(const char* name, const processor_config_t* config, int sample_rate, size_t chunk_size);

/**
 * @brief Wraps an existing compressor processor into the generic dsp_processor_t interface.
 *
...
/**
 * @brief Wraps an existing noise gate processor into the generic dsp_processor_t interface.
 *
...
/**
 * @brief Wraps an existing RACE processor into the generic dsp_processor_t interface.
 *
...
/// Apply the processor to all channels of `chunk` in place.
static inline void dsp_processor_process(dsp_processor_t* proc, audio_chunk_t* chunk) {
...
/// Update the processor parameters dynamically.
static inline void dsp_processor_update_parameters(dsp_processor_t* proc, const processor_config_t* config, int sample_rate) {
