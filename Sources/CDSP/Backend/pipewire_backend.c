...
static void on_capture_process(void* data) {
    ...
    if (src) {
        ...
        spsc_audio_ring_buffer_write(c->ring, src + (offset / sizeof(float)), frames * c->channels, 1);
    }
...
static bool cap_vtable_get_pending_rate_change(void* ctx, double* out_rate) {
    return pipewire_capture_get_pending_rate_change((pipewire_capture_t*)ctx, out_rate);
}
...
capture_backend_t* pipewire_capture_create(const capture_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    pipewire_capture_t* capture = (pipewire_capture_t*)calloc(1, sizeof(pipewire_capture_t));
...
bool pipewire_capture_open(pipewire_capture_t* capture, backend_error_t* err) {
    ...
    if (!capture->loop) {
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to create PipeWire thread loop");
        return false;
    }
    ...
    if (pw_thread_loop_start(capture->loop) < 0) {
        ...
        capture->loop = NULL;
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to start PipeWire thread loop");
        return false;
    }
    ...
    if (!capture->context) {
        ...
        pw_thread_loop_destroy(capture->loop);
        capture->loop = NULL;
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to create PipeWire context");
        return false;
    }
    ...
    if (!capture->stream) {
        ...
        pw_thread_loop_destroy(capture->loop);
        capture->loop = NULL;
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to create PipeWire stream");
        return false;
    }
    ...
    if (rc < 0) {
        ...
        pw_thread_loop_destroy(capture->loop);
        capture->loop = NULL;
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to connect PipeWire stream");
        return false;
    }
    
    // Create large SPSC ring buffer (8 blocks headroom)
    capture->ring = spsc_audio_ring_buffer_create(capture->chunk_size * capture->channels * 8);
    capture->decode_buf_size = capture->chunk_size * capture->channels;
    capture->decode_buf = (float*)malloc(capture->decode_buf_size * sizeof(float));
    
    logger_t logger = logger_create("dsp.backend.pipewire");
    logger_info(&logger, "Opened PipeWire capture: device=%s, rate=%d, channels=%d",
...
bool pipewire_capture_read(pipewire_capture_t* capture, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
    ...
    while (spsc_audio_ring_buffer_get_available_to_read(capture->ring) < requested) {
        struct timespec req = { .tv_sec = 0, .tv_nsec = 1000000L }; // 1ms
        nanosleep(&req, NULL);
    }
    
    size_t consumed = spsc_audio_ring_buffer_consume(capture->ring, capture->decode_buf, requested);
    if (consumed < requested) {
...
static bool play_vtable_get_pending_rate_change(void* ctx, double* out_rate) {
    return pipewire_playback_get_pending_rate_change((pipewire_playback_t*)ctx, out_rate);
}
static bool play_vtable_prefill_silence(void* ctx, size_t frames, backend_error_t* err) {
    return pipewire_playback_prefill_silence((pipewire_playback_t*)ctx, frames, err);
}
...
playback_backend_t* pipewire_playback_create(const playback_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    pipewire_playback_t* playback = (pipewire_playback_t*)calloc(1, sizeof(pipewire_playback_t));
...
bool pipewire_playback_open(pipewire_playback_t* playback, backend_error_t* err) {
    ...
    playback->context = pw_context_new(pw_thread_loop_get_loop(playback->loop), NULL, 0);
    if (!playback->context) {
    ...
    playback->stream = pw_stream_new_simple(
        pw_thread_loop_get_loop(playback->loop),
        "CDSP-Playback-Stream",
        pw_properties_new(
            PW_KEY_MEDIA_TYPE, "Audio",
    ...
    int rc = pw_stream_connect(
        playback->stream,
        PW_DIRECTION_OUTPUT,
        playback->device[0] != '\0' ? (uint32_t)atoi(playback->device) : PW_ID_ANY,
        PW_STREAM_FLAG_AUTOCONNECT |
        PW_STREAM_FLAG_MAP_BUFFERS |
        PW_STREAM_FLAG_RT_PROCESS,
        params, 1
    ...
    playback->ring = spsc_audio_ring_buffer_create(playback->chunk_size * playback->channels * 8);
    playback->encode_buf_size = playback->chunk_size * playback->channels;
    playback->encode_buf = (float*)malloc(playback->encode_buf_size * sizeof(float));
    playback->paused = false;
    
    logger_t logger = logger_create("dsp.backend.pipewire");
    logger_info(&logger, "Opened PipeWire playback: device=%s, rate=%d, channels=%d",
...
bool pipewire_playback_write(pipewire_playback_t* playback, const audio_chunk_t* chunk, backend_error_t* err) {
    ...
    // Wait until there is space in the SPSC ring buffer (to prevent overwriting oldest data)
    while (spsc_audio_ring_buffer_get_available_to_read(playback->ring) + requested > playback->ring->capacity) {
        struct timespec req = { .tv_sec = 0, .tv_nsec = 1000000L }; // 1ms
        nanosleep(&req, NULL);
    }
    
    spsc_audio_ring_buffer_write(playback->ring, playback->encode_buf, requested, 1);
    return true;
}
...
void pipewire_playback_close(pipewire_playback_t* playback) {
    if (playback->loop) {
        ...
        while (spsc_audio_ring_buffer_get_available_to_read(playback->ring) > 0 && retries-- > 0) {
            struct timespec req = { .tv_sec = 0, .tv_nsec = 1000000L };
            nanosleep(&req, NULL);
        }
...
size_t pipewire_playback_get_buffer_level(pipewire_playback_t* playback) {
    ...
    return spsc_audio_ring_buffer_get_available_to_read(playback->ring) / playback->channels;
}
...
bool pipewire_playback_prefill_silence(pipewire_playback_t* playback, size_t frames, backend_error_t* err) {
    ...
    spsc_audio_ring_buffer_write_silence(playback->ring, frames * playback->channels);
    return true;
}
