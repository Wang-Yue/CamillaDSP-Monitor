...
static size_t get_sample_size(wasapi_sample_format_t format) {
    switch (format) {
        case WASAPI_SAMPLE_FORMAT_S16: return 2;
        case WASAPI_SAMPLE_FORMAT_S24: return 4; // 24-bit in 32-bit container
        case WASAPI_SAMPLE_FORMAT_S32: return 4;
        case WASAPI_SAMPLE_FORMAT_F32: return 4;
        default: return 0;
    }
}

static inline void decode_samples_from_wasapi(audio_chunk_t* chunk, size_t chunk_offset, const BYTE* src, size_t frames, wasapi_sample_format_t format, int channels, DWORD flags) {
    ...
    switch (format) {
        case WASAPI_SAMPLE_FORMAT_S16: {
            ...
            for (size_t f = 0; f < frames; f++) {
                for (int c = 0; c < channels; c++) {
                    audio_chunk_get_channel(chunk, c)[chunk_offset + f] = (double)s16[f * channels + c] / 32768.0;
                }
            }
            break;
        }
        case WASAPI_SAMPLE_FORMAT_S24: {
            ...
            for (size_t f = 0; f < frames; f++) {
                for (int c = 0; c < channels; c++) {
                    int32_t val = s32[f * channels + c] >> 8; // shift right to match 24-bit range
                    audio_chunk_get_channel(chunk, c)[chunk_offset + f] = (double)val / 8388608.0;
                }
            }
            break;
        }
        case WASAPI_SAMPLE_FORMAT_S32: {
            ...
            for (size_t f = 0; f < frames; f++) {
                for (int c = 0; c < channels; c++) {
                    audio_chunk_get_channel(chunk, c)[chunk_offset + f] = (double)s32[f * channels + c] / 2147483648.0;
                }
            }
            break;
        }
        case WASAPI_SAMPLE_FORMAT_F32: {
            ...
            for (size_t f = 0; f < frames; f++) {
                for (int c = 0; c < channels; c++) {
                    audio_chunk_get_channel(chunk, c)[chunk_offset + f] = (double)f32[f * channels + c];
                }
            }
            break;
        }
...
static inline void encode_samples_to_wasapi(BYTE* dst, const audio_chunk_t* chunk, size_t chunk_offset, size_t frames, wasapi_sample_format_t format, int channels) {
    switch (format) {
        case WASAPI_SAMPLE_FORMAT_S16: {
        ...
        case WASAPI_SAMPLE_FORMAT_F32: {
            ...
            for (size_t f = 0; f < frames; f++) {
                for (int c = 0; c < channels; c++) {
                    f32[f * channels + c] = (float)audio_chunk_get_channel(chunk, c)[chunk_offset + f];
                }
            }
            break;
        }
...
static bool cap_vtable_read(void* ctx, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
...
static bool cap_vtable_get_pending_rate_change(void* ctx, double* out_rate) {
    return wasapi_capture_get_pending_rate_change((wasapi_capture_t*)ctx, out_rate);
}
...
capture_backend_t* wasapi_capture_create(const capture_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    wasapi_capture_t* capture = (wasapi_capture_t*)calloc(1, sizeof(wasapi_capture_t));
...
bool wasapi_capture_open(wasapi_capture_t* capture, backend_error_t* err) {
    ...
    HRESULT hr = CoCreateInstance(&CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, &IID_IMMDeviceEnumerator, (void**)&capture->enumerator);
    if (FAILED(hr)) {
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to create MMDeviceEnumerator");
        return false;
    }
    
    if (capture->device[0] == '\0') {
        hr = IMMDeviceEnumerator_GetDefaultAudioEndpoint(capture->enumerator, capture->loopback ? eRender : eCapture, eConsole, &capture->mm_device);
    } else {
        IMMDeviceCollection* collection = NULL;
        hr = IMMDeviceEnumerator_EnumAudioEndpoints(capture->enumerator, capture->loopback ? eRender : eCapture, DEVICE_STATE_ACTIVE, &collection);
        if (SUCCEEDED(hr)) {
            UINT count = 0;
    ...
    if (!capture->mm_device) {
        ...
        if (err) backend_error_init(err, BACKEND_ERROR_DEVICE_NOT_FOUND, "WASAPI capture device not found");
        return false;
    }
    
    hr = IMMDevice_Activate(capture->mm_device, &IID_IAudioClient, CLSCTX_ALL, NULL, (void**)&capture->client);
    if (FAILED(hr)) {
        SAFE_RELEASE(capture->mm_device);
        SAFE_RELEASE(capture->enumerator);
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to activate IAudioClient");
        return false;
    }
    ...
    wfx.Format.wBitsPerSample = (capture->format == WASAPI_SAMPLE_FORMAT_S16) ? 16 : 32;
    wfx.Format.nBlockAlign = (wfx.Format.wBitsPerSample / 8) * capture->channels;
    wfx.Format.nAvgBytesPerSec = wfx.Format.nSamplesPerSec * wfx.Format.nBlockAlign;
    wfx.Format.cbSize = 22;
    wfx.Samples.wValidBitsPerSample = (capture->format == WASAPI_SAMPLE_FORMAT_S24) ? 24 : wfx.Format.wBitsPerSample;
    wfx.dwChannelMask = (capture->channels == 2) ? (SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT) : 0;
    wfx.SubFormat = (capture->format == WASAPI_SAMPLE_FORMAT_F32) ? KSDATAFORMAT_SUBTYPE_IEEE_FLOAT : KSDATAFORMAT_SUBTYPE_PCM;
    
    REFERENCE_TIME duration = 10000000; // 1 second buffer
    DWORD flags = capture->loopback ? AUDCLNT_STREAMFLAGS_LOOPBACK : 0;
    
    hr = IAudioClient_Initialize(capture->client, AUDCLNT_SHAREMODE_SHARED, flags, duration, 0, (WAVEFORMATEX*)&wfx, NULL);
    if (FAILED(hr)) {
        SAFE_RELEASE(capture->client);
        SAFE_RELEASE(capture->mm_device);
        SAFE_RELEASE(capture->enumerator);
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to initialize IAudioClient");
        return false;
    }
    
    hr = IAudioClient_GetBufferSize(capture->client, &capture->buffer_frame_count);
    hr = IAudioClient_GetService(capture->client, &IID_IAudioCaptureClient, (void**)&capture->capture_client);
    if (FAILED(hr)) {
        SAFE_RELEASE(capture->client);
        SAFE_RELEASE(capture->mm_device);
        SAFE_RELEASE(capture->enumerator);
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to get IAudioCaptureClient");
        return false;
    }
    ...
    logger_info(&logger, "Opened WASAPI capture: device=%s, rate=%d, channels=%d, loopback=%d",
...
bool wasapi_capture_read(wasapi_capture_t* capture, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
    size_t frames_read = 0;
    
    while (frames_read < frames) {
        UINT32 packet_size = 0;
        HRESULT hr = IAudioCaptureClient_GetNextPacketSize(capture->capture_client, &packet_size);
        if (FAILED(hr)) {
            if (err) backend_error_init(err, BACKEND_ERROR_READ_ERROR, "Failed to get packet size");
            return false;
        }
        ...
        if (packet_size > 0) {
            ...
            UINT32 num_frames = 0;
            DWORD flags = 0;
            hr = IAudioCaptureClient_GetBuffer(capture->capture_client, &data, &num_frames, &flags, NULL, NULL);
            if (SUCCEEDED(hr) && data) {
                UINT32 to_copy = frames - frames_read;
                if (to_copy > num_frames) to_copy = num_frames;
                
                decode_samples_from_wasapi(chunk, frames_read, data, to_copy, capture->format, capture->channels, flags);
...
bool wasapi_capture_get_pending_rate_change(wasapi_capture_t* capture, double* out_rate) {
    (void)capture;
    (void)out_rate;
    return false;
}
...
bool wasapi_capture_wait(wasapi_capture_t* capture, uint32_t timeout_ms) {
    ...
    while (elapsed < timeout_ms) {
        UINT32 packet_size = 0;
        IAudioCaptureClient_GetNextPacketSize(capture->capture_client, &packet_size);
...
static bool play_vtable_write(void* ctx, const audio_chunk_t* chunk, backend_error_t* err) {
...
static bool play_vtable_get_pending_rate_change(void* ctx, double* out_rate) {
    return wasapi_playback_get_pending_rate_change((wasapi_playback_t*)ctx, out_rate);
}
static bool play_vtable_prefill_silence(void* ctx, size_t frames, backend_error_t* err) {
...
playback_backend_t* wasapi_playback_create(const playback_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    wasapi_playback_t* playback = (wasapi_playback_t*)calloc(1, sizeof(wasapi_playback_t));
...
bool wasapi_playback_open(wasapi_playback_t* playback, backend_error_t* err) {
    ...
    HRESULT hr = CoCreateInstance(&CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, &IID_IMMDeviceEnumerator, (void**)&playback->enumerator);
    if (FAILED(hr)) {
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to create MMDeviceEnumerator");
        return false;
    }
    ...
    if (playback->device[0] == '\0') {
        hr = IMMDeviceEnumerator_GetDefaultAudioEndpoint(playback->enumerator, eRender, eConsole, &playback->mm_device);
    } else {
        IMMDeviceCollection* collection = NULL;
        hr = IMMDeviceEnumerator_EnumAudioEndpoints(playback->enumerator, eRender, DEVICE_STATE_ACTIVE, &collection);
        if (SUCCEEDED(hr)) {
            UINT count = 0;
            IMMDeviceCollection_GetCount(collection, &count);
            for (UINT i = 0; i < count; i++) {
    ...
    if (!playback->mm_device) {
        ...
        if (err) backend_error_init(err, BACKEND_ERROR_DEVICE_NOT_FOUND, "WASAPI playback device not found");
        return false;
    }
    
    hr = IMMDevice_Activate(playback->mm_device, &IID_IAudioClient, CLSCTX_ALL, NULL, (void**)&playback->client);
    if (FAILED(hr)) {
    ...
    wfx.Format.wBitsPerSample = (playback->format == WASAPI_SAMPLE_FORMAT_S16) ? 16 : 32;
    wfx.Format.nBlockAlign = (wfx.Format.wBitsPerSample / 8) * playback->channels;
    wfx.Format.nAvgBytesPerSec = wfx.Format.nSamplesPerSec * wfx.Format.nBlockAlign;
    wfx.Format.cbSize = 22;
    wfx.Samples.wValidBitsPerSample = (playback->format == WASAPI_SAMPLE_FORMAT_S24) ? 24 : wfx.Format.wBitsPerSample;
    wfx.dwChannelMask = (playback->channels == 2) ? (SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT) : 0;
    wfx.SubFormat = (playback->format == WASAPI_SAMPLE_FORMAT_F32) ? KSDATAFORMAT_SUBTYPE_IEEE_FLOAT : KSDATAFORMAT_SUBTYPE_PCM;
    
    REFERENCE_TIME duration = 10000000;
    AUDCLNT_SHAREMODE mode = playback->exclusive ? AUDCLNT_SHAREMODE_EXCLUSIVE : AUDCLNT_SHAREMODE_SHARED;
    
    hr = IAudioClient_Initialize(playback->client, mode, 0, duration, playback->exclusive ? duration : 0, (WAVEFORMATEX*)&wfx, NULL);
    if (FAILED(hr)) {
    ...
    hr = IAudioClient_GetBufferSize(playback->client, &playback->buffer_frame_count);
    hr = IAudioClient_GetService(playback->client, &IID_IAudioRenderClient, (void**)&playback->render_client);
    if (FAILED(hr)) {
        SAFE_RELEASE(playback->client);
        SAFE_RELEASE(playback->mm_device);
        SAFE_RELEASE(playback->enumerator);
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to get IAudioRenderClient");
        return false;
    }
    ...
    logger_t logger = logger_create("dsp.backend.wasapi");
    logger_info(&logger, "Opened WASAPI playback: device=%s, rate=%d, channels=%d, exclusive=%d",
                log_arg_string(playback->device[0] != '\0' ? playback->device : "default"),
...
bool wasapi_playback_write(wasapi_playback_t* playback, const audio_chunk_t* chunk, backend_error_t* err) {
    ...
    while (frames_written < total_frames) {
        ...
        if (FAILED(hr)) {
            if (err) backend_error_init(err, BACKEND_ERROR_WRITE_ERROR, "Failed to get padding");
            return false;
        }
        ...
        if (to_write > 0) {
            ...
            if (SUCCEEDED(hr) && data) {
                encode_samples_to_wasapi(data, chunk, frames_written, to_write, playback->format, playback->channels);
...
bool wasapi_playback_get_pending_rate_change(wasapi_playback_t* playback, double* out_rate) {
    (void)playback;
    (void)out_rate;
    return false;
}

bool wasapi_playback_prefill_silence(wasapi_playback_t* playback, size_t frames, backend_error_t* err) {
    if (!playback->render_client) return false;
    BYTE* data = NULL;
    HRESULT hr = IAudioRenderClient_GetBuffer(playback->render_client, (UINT32)frames, &data);
    if (SUCCEEDED(hr) && data) {
        memset(data, 0, frames * playback->channels * get_sample_size(playback->format));
        IAudioRenderClient_ReleaseBuffer(playback->render_client, (UINT32)frames, 0);
        return true;
    }
    if (err) backend_error_init(err, BACKEND_ERROR_WRITE_ERROR, "Failed to prefill silence");
    return false;
}
