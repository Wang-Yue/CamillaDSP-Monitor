...
int core_audio_capabilities_available_device_names(bool is_capture, char out_names[][256], int max_names) {
    core_audio_scope_t scope = is_capture ? CORE_AUDIO_SCOPE_INPUT : CORE_AUDIO_SCOPE_OUTPUT;
...
int core_audio_capabilities_channel_count(const char* device_name, bool is_capture) {
    audio_device_descriptor_t* desc = core_audio_capabilities_describe(device_name, is_capture);
    if (!desc) return 0;
    int max_ch = 0;
...
static const char* format_string_for_asbd(const AudioStreamBasicDescription* asbd) {
...
audio_device_descriptor_t* core_audio_capabilities_describe(const char* device_name, bool is_capture) {
    ...
    for (int s = 0; s < stream_count; s++) {
        ...
        uint32_t size = 0;
        if (AudioObjectGetPropertyDataSize(streams[s], &addr, 0, NULL, &size) != noErr || size == 0) continue;
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (lo == hi) {
                    ...
                    for (size_t r = 0; r < CORE_AUDIO_STANDARD_RATES_COUNT; r++) {
                        if ((double)CORE_AUDIO_STANDARD_RATES[r] >= lo && (double)CORE_AUDIO_STANDARD_RATES[r] <= hi) {
                    ...
                    if (hint > 0) {
                        ...
                        for (int k = 0; k < rate_cnt; k++) {
                            if (rates_to_add[k] == hint) { found = true; break; }
                        }
    ...
    for (int i = 0; i < fmt_count; i++) {
        ...
        for (int j = 0; j < unique_ch_cnt; j++) {
            if (unique_ch[j] == fmts[i].channels) { found = true; break; }
        }
    ...
    set->capabilities = (channel_capability_t*)calloc(unique_ch_cnt, sizeof(channel_capability_t));
    ...
    for (int c = 0; c < unique_ch_cnt; c++) {
        ...
        for (int i = 0; i < fmt_count; i++) {
            if (fmts[i].channels == ch_cap->channels) {
                ...
                for (int j = 0; j < unique_rate_cnt; j++) {
                    if (unique_rate[j] == fmts[i].samplerate) { found = true; break; }
                }
        ...
        ch_cap->samplerates = (samplerate_capability_t*)calloc(unique_rate_cnt, sizeof(samplerate_capability_t));
        ...
        for (int r = 0; r < unique_rate_cnt; r++) {
            ...
            for (int i = 0; i < fmt_count; i++) {
                if (fmts[i].channels == ch_cap->channels && fmts[i].samplerate == rate_cap->samplerate) {
                    bool found = false;
                    for (int j = 0; j < unique_fmt_cnt; j++) {
                        if (strcmp(unique_fmt[j], fmts[i].format) == 0) { found = true; break; }
                    }
