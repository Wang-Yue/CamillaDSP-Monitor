...
void backend_error_init(backend_error_t* err, backend_error_type_t type, const char* message) {
    if (!err) return;
    err->type = type;
    if (message) {
...
const char* backend_error_description(const backend_error_t* err, char* out_buf, size_t buf_len) {
