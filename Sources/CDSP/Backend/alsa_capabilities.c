...
const int ALSA_PROBE_RATES[] = { 44100, 48000, 88200, 96000, 176400, 192000, 352800, 384000 };
const size_t ALSA_PROBE_RATES_COUNT = sizeof(ALSA_PROBE_RATES) / sizeof(ALSA_PROBE_RATES[0]);

int alsa_capabilities_available_device_names(bool is_capture, char out_names[][256], int max_names) {
    (void)is_capture;
    int count = 0;
    if (count < max_names) {
...
bool alsa_capabilities_default_device_name(bool is_capture, char* out_name, size_t max_len) {
    (void)is_capture;
...
int alsa_capabilities_channel_count(const char* device_name, bool is_capture) {
    snd_pcm_t* pcm = NULL;
    snd_pcm_stream_t stream = is_capture ? SND_PCM_STREAM_CAPTURE : SND_PCM_STREAM_PLAYBACK;
    char clean_name[256];
...
audio_device_descriptor_t* alsa_capabilities_describe(const char* device_name, bool is_capture) {
    ...
    // Let's create capabilities for all channel counts from min_ch to max_ch (up to 32)
    size_t cap_idx = 0;
    size_t cap_alloc = (max_ch - min_ch + 1);
    set->capabilities = (channel_capability_t*)calloc(cap_alloc, sizeof(channel_capability_t));
    ...
    for (unsigned int ch = min_ch; ch <= max_ch; ch++) {
        ...
        cap->samplerates = (samplerate_capability_t*)calloc(ALSA_PROBE_RATES_COUNT, sizeof(samplerate_capability_t));
        size_t rate_idx = 0;
        ...
        for (size_t r = 0; r < ALSA_PROBE_RATES_COUNT; r++) {
            ...
            if (snd_pcm_hw_params_set_rate(pcm, params, test_rate, 0) >= 0) {
                // Rate is supported! Now probe which formats are supported for this rate and channel count
