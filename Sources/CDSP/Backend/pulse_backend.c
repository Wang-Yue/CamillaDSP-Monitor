...
capture_backend_t* pulse_capture_create(const capture_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    pulse_capture_t* capture = (pulse_capture_t*)calloc(1, sizeof(pulse_capture_t));
...
bool pulse_capture_open(pulse_capture_t* capture, backend_error_t* err) {
    ...
    if (!capture->s) {
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, pa_strerror(error));
        return false;
    }
    
    capture->raw_buf_size = capture->chunk_size * capture->channels * sizeof(float);
    ...
    logger_info(&logger, "Opened PulseAudio capture: device=%s, rate=%d, channels=%d",
...
bool pulse_capture_read(pulse_capture_t* capture, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
    ...
    if (pa_simple_read(capture->s, capture->raw_buf, bytes_to_read, &error) < 0) {
        if (err) backend_error_init(err, BACKEND_ERROR_READ_ERROR, pa_strerror(error));
        return false;
    }
    ...
    for (size_t f = 0; f < frames; f++) {
        for (int c = 0; c < capture->channels; c++) {
            audio_chunk_get_channel(chunk, c)[f] = (double)src[f * capture->channels + c];
        }
    }
...
static bool play_vtable_get_pending_rate_change(void* ctx, double* out_rate) {
    return pulse_playback_get_pending_rate_change((pulse_playback_t*)ctx, out_rate);
}
...
playback_backend_t* pulse_playback_create(const playback_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    pulse_playback_t* playback = (pulse_playback_t*)calloc(1, sizeof(pulse_playback_t));
...
bool pulse_playback_open(pulse_playback_t* playback, backend_error_t* err) {
    ...
    playback->raw_buf_size = playback->chunk_size * playback->channels * sizeof(float);
    ...
    logger_t logger = logger_create("dsp.backend.pulse");
    logger_info(&logger, "Opened PulseAudio playback: device=%s, rate=%d, channels=%d",
...
bool pulse_playback_write(pulse_playback_t* playback, const audio_chunk_t* chunk, backend_error_t* err) {
    ...
    for (size_t f = 0; f < frames; f++) {
        for (int c = 0; c < playback->channels; c++) {
            dst[f * playback->channels + c] = (float)audio_chunk_get_channel(chunk, c)[f];
        }
    }
    
    int error;
    if (pa_simple_write(playback->s, playback->raw_buf, required_bytes, &error) < 0) {
        if (err) backend_error_init(err, BACKEND_ERROR_WRITE_ERROR, pa_strerror(error));
        return false;
    }
...
bool pulse_playback_prefill_silence(pulse_playback_t* playback, size_t frames, backend_error_t* err) {
    ...
    if (!silence) {
        if (err) backend_error_init(err, BACKEND_ERROR_WRITE_ERROR, "Memory allocation failure");
        return false;
    }
