...
bool capture_backend_read(capture_backend_t* backend, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
...
bool capture_backend_get_pending_rate_change(capture_backend_t* backend, double* out_rate) {
    if (!backend || !backend->vtable || !backend->vtable->get_pending_rate_change) return false;
...
bool capture_backend_pitch_control_supported(capture_backend_t* backend) {
    if (!backend || !backend->vtable || !backend->vtable->is_pitch_control_supported) return false;
...
bool capture_backend_wait(capture_backend_t* backend, uint32_t timeout_ms) {
    if (!backend || !backend->vtable || !backend->vtable->wait_for_data) return false;
...
bool playback_backend_write(playback_backend_t* backend, const audio_chunk_t* chunk, backend_error_t* err) {
...
size_t playback_backend_get_buffer_level(playback_backend_t* backend) {
    if (!backend || !backend->vtable || !backend->vtable->get_buffer_level) return 0;
    return backend->vtable->get_buffer_level(backend->ctx);
}

/// Get pending sample rate change detected on the playback device.
bool playback_backend_get_pending_rate_change(playback_backend_t* backend, double* out_rate) {
    if (!backend || !backend->vtable || !backend->vtable->get_pending_rate_change) return false;
    return backend->vtable->get_pending_rate_change(backend->ctx, out_rate);
}

/// Push zero samples per channel into the output ring before first real chunk arrives.
bool playback_backend_prefill_silence(playback_backend_t* backend, size_t frames, backend_error_t* err) {
    if (!backend || !backend->vtable || !backend->vtable->prefill_silence) return false;
