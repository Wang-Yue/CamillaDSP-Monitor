...
static size_t get_sample_size(binary_sample_format_t format) {
    switch (format) {
        ...
        case BINARY_SAMPLE_FORMAT_S24_4_RJ_LE: return 4;
        case BINARY_SAMPLE_FORMAT_S24_4_LJ_LE: return 4;
...
static bool parse_wav_header(FILE* f, wav_info_t* info, char* err_msg, size_t err_msg_len) {
    uint8_t header[44];
    ...
    if (audio_format != 1 && audio_format != 3) {
        snprintf(err_msg, err_msg_len, "Unsupported audio format %d (only PCM/Float supported)", audio_format);
        return false;
    }
    
    binary_sample_format_t format = BINARY_SAMPLE_FORMAT_INVALID;
    if (audio_format == 1) {
        if (bits_per_sample == 16) format = BINARY_SAMPLE_FORMAT_S16_LE;
        else if (bits_per_sample == 24) format = BINARY_SAMPLE_FORMAT_S24_3_LE;
        else if (bits_per_sample == 32) format = BINARY_SAMPLE_FORMAT_S32_LE;
    } else if (audio_format == 3) {
        if (bits_per_sample == 32) format = BINARY_SAMPLE_FORMAT_F32_LE;
        else if (bits_per_sample == 64) format = BINARY_SAMPLE_FORMAT_F64_LE;
    }
    
    if (format == BINARY_SAMPLE_FORMAT_INVALID) {
        snprintf(err_msg, err_msg_len, "Unsupported bits per sample %d", bits_per_sample);
        return false;
    }
...
static void write_wav_header_to_file(FILE* f, size_t channels, binary_sample_format_t format, uint32_t sample_rate, uint32_t data_bytes) {
    uint8_t header[44];
    memset(header, 0, 44);
    ...
    header[16] = 16;
    uint16_t format_tag = (format == BINARY_SAMPLE_FORMAT_F32_LE || format == BINARY_SAMPLE_FORMAT_F64_LE) ? 3 : 1;
...
static inline double decode_sample(const uint8_t* src, binary_sample_format_t format) {
    switch (format) {
...
static inline void encode_sample(uint8_t* dst, double value, binary_sample_format_t format) {
...
capture_backend_t* file_capture_create(const capture_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    ...
    if (config->type == AUDIO_BACKEND_TYPE_STDIN_OUT) {
        ...
        snprintf(capture->filename, sizeof(capture->filename), "%s", config->filename);
        capture->format = config->file_format;
    ...
    capture->extra_samples = config->has_extra_samples ? (size_t)config->extra_samples : 0;
...
bool file_capture_open(file_capture_t* capture, backend_error_t* err) {
    if (capture->is_stdin) {
        ...
        if (!capture->f) {
            if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to open input file");
            return false;
        }
    }
    ...
    if (capture->is_wav && !capture->is_stdin) {
        ...
        logger_info(&logger, "Parsed input WAV file: rate=%d Hz, channels=%d, format=%s",
    ...
    capture->raw_buf_capacity = capture->chunk_size * capture->channels * sample_size;
    capture->raw_buf = (uint8_t*)malloc(capture->raw_buf_capacity);
    if (!capture->raw_buf) {
        if (!capture->is_stdin) fclose(capture->f);
        if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Memory allocation failure");
        return false;
    }
...
bool file_capture_read(file_capture_t* capture, size_t frames, audio_chunk_t* chunk, backend_error_t* err) {
    (void)err;
    size_t sample_size = get_sample_size(capture->format);
    size_t frames_to_read = frames;
    
    size_t bytes_to_read = frames_to_read * capture->channels * sample_size;
    if (capture->read_bytes > 0 && (capture->total_bytes_read + bytes_to_read) > capture->read_bytes) {
    ...
    for (size_t f = 0; f < frames_read; f++) {
        for (int c = 0; c < capture->channels; c++) {
            ...
            audio_chunk_get_channel(chunk, c)[f] = decode_sample(capture->raw_buf + offset, capture->format);
        }
    }
    ...
    if (frames_read < frames) {
        ...
        size_t extra_to_generate = capture->extra_samples - capture->extra_samples_generated;
...
bool file_capture_get_pending_rate_change(file_capture_t* capture, double* out_rate) {
    (void)capture;
    (void)out_rate;
    return false;
}
...
playback_backend_t* file_playback_create(const playback_device_config_t* config, int sample_rate, int chunk_size, processing_parameters_t* params, backend_error_t* err) {
    (void)params;
    (void)err;
    file_playback_t* playback = (file_playback_t*)calloc(1, sizeof(file_playback_t));
    ...
    if (config->type == AUDIO_BACKEND_TYPE_STDIN_OUT) {
        ...
        snprintf(playback->filename, sizeof(playback->filename), "%s", config->filename);
        playback->format = config->file_format;
        playback->is_wav = config->is_wav;
    }
...
bool file_playback_open(file_playback_t* playback, backend_error_t* err) {
    if (playback->is_stdout) {
        ...
        if (!playback->f) {
            if (err) backend_error_init(err, BACKEND_ERROR_INITIALIZATION_FAILED, "Failed to open output file");
            return false;
        }
    }
    
    size_t sample_size = get_sample_size(playback->format);
    playback->raw_buf_capacity = playback->chunk_size * playback->channels * sample_size;
...
bool file_playback_write(file_playback_t* playback, const audio_chunk_t* chunk, backend_error_t* err) {
    (void)err;
    ...
    for (size_t f = 0; f < frames; f++) {
        for (int c = 0; c < playback->channels; c++) {
            ...
            encode_sample(playback->raw_buf + offset, audio_chunk_get_channel(chunk, c)[f], playback->format);
        }
    }
    
    size_t bytes_written = fwrite(playback->raw_buf, 1, required_bytes, playback->f);
...
void file_playback_close(file_playback_t* playback) {
    if (playback->f) {
        if (playback->is_wav && !playback->is_stdout) {
            ...
            write_wav_header_to_file(playback->f, playback->channels, playback->format, playback->sample_rate, playback->total_bytes_written);
        }
        if (!playback->is_stdout) {
            fclose(playback->f);
        }
...
bool file_playback_get_pending_rate_change(file_playback_t* playback, double* out_rate) {
    (void)playback;
    (void)out_rate;
    return false;
}

bool file_playback_prefill_silence(file_playback_t* playback, size_t frames, backend_error_t* err) {
    (void)err;
    // For file playback, we don't need to prefill silence because it is not real-time audio.
    // However, to keep it clean and avoid errors, we just return true.
    (void)playback;
    (void)frames;
    return true;
}
