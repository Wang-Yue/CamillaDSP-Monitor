...
const char* apple_resampler_quality_to_string(apple_resampler_quality_t quality) {
    switch (quality) {
        case APPLE_RESAMPLER_QUALITY_MIN: return "Min";
        case APPLE_RESAMPLER_QUALITY_LOW: return "Low";
        case APPLE_RESAMPLER_QUALITY_MEDIUM: return "Medium";
        case APPLE_RESAMPLER_QUALITY_HIGH: return "High";
        case APPLE_RESAMPLER_QUALITY_MAX: return "Max";
...
const char* apple_resampler_complexity_to_string(apple_resampler_complexity_t comp) {
    switch (comp) {
        case APPLE_RESAMPLER_COMPLEXITY_LINEAR: return "Linear";
        case APPLE_RESAMPLER_COMPLEXITY_NORMAL: return "Normal";
        case APPLE_RESAMPLER_COMPLEXITY_MASTERING: return "Mastering";
        case APPLE_RESAMPLER_COMPLEXITY_MINIMUM_PHASE: return "MinimumPhase";
...
apple_resampler_complexity_t apple_resampler_complexity_from_string(const char* str) {
    ...
    if (strcmp(str, "Mastering") == 0) return APPLE_RESAMPLER_COMPLEXITY_MASTERING;
    if (strcmp(str, "MinimumPhase") == 0) return APPLE_RESAMPLER_COMPLEXITY_MINIMUM_PHASE;
    return APPLE_RESAMPLER_COMPLEXITY_NORMAL;
}

uint32_t apple_resampler_complexity_os_type(apple_resampler_complexity_t comp) {
    switch (comp) {
        case APPLE_RESAMPLER_COMPLEXITY_LINEAR: return 0x6C696E65; // 'line'
        case APPLE_RESAMPLER_COMPLEXITY_NORMAL: return 0x6E6F726D; // 'norm'
        case APPLE_RESAMPLER_COMPLEXITY_MASTERING: return 0x62617473; // 'bats'
        case APPLE_RESAMPLER_COMPLEXITY_MINIMUM_PHASE: return 0x6D696E70; // 'minp'
...
void resampler_config_description(const resampler_config_t* config, char* out_buf, size_t buf_len) {
    ...
    const char* interp = config->has_interpolation ? config->interpolation : "nil";
    int sinc = config->has_sinc_len ? config->sinc_len : 0;
    snprintf(out_buf, buf_len, "ResamplerConfig(type: %s, profile: %s, interpolation: %s, sincLen: %d)",
