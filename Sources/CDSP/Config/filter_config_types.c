...
const char* filter_type_to_string(filter_type_t type) {
    switch (type) {
        ...
        case FILTER_TYPE_LIMITER: return "Limiter";
...
int gain_parameters_validate(const gain_parameters_t* params, config_error_t* err) {
    ...
    if (params->has_gain) {
        if (params->gain <= -150.0 || params->gain >= 150.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "gain must be in (-150, 150) dB, got %g", params->gain);
            return -1;
        }
    }
    return 0;
...
int loudness_parameters_validate(const loudness_parameters_t* params, config_error_t* err) {
    ...
    if (params->has_reference_level) {
        if (params->reference_level <= -100.0 || params->reference_level >= 20.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "reference_level must be in (-100, 20), got %g", params->reference_level);
            return -1;
        }
    }
    if (params->has_high_boost) {
        if (params->high_boost < 0.0 || params->high_boost > 20.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "high_boost must be in [0, 20], got %g", params->high_boost);
            return -1;
        }
    }
    if (params->has_low_boost) {
        if (params->low_boost < 0.0 || params->low_boost > 20.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "low_boost must be in [0, 20], got %g", params->low_boost);
            return -1;
        }
    }
    return 0;
    ...
    switch (params->type) {
        case CONV_TYPE_VALUES:
            if (!params->values || params->values_count == 0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Conv 'values' must be non-empty");
                return -1;
            }
            break;
        case CONV_TYPE_WAV:
            if (params->filename[0] == '\0') {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Conv 'Wav' missing filename");
                return -1;
            }
            break;
        case CONV_TYPE_RAW:
            if (params->filename[0] == '\0') {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Conv 'Raw' missing filename");
                return -1;
            }
            break;
        case CONV_TYPE_DUMMY:
            if (params->length <= 0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Conv 'dummy' length must be > 0");
                return -1;
            }
            break;
    }
...
int delay_parameters_validate(const delay_parameters_t* params, config_error_t* err) {
    if (!params) return 0;
    if (params->delay < 0.0) {
        config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Delay cannot be negative, got %g", params->delay);
        return -1;
    }
    return 0;
}
    ...
    switch (params->type) {
        ...
        case BIQUAD_COMBO_TYPE_BUTTERWORTH_HIGHPASS:
            if (!params->has_freq || params->freq <= 0.0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "BiquadCombo: freq must be > 0, got %g", params->freq);
                return -1;
            }
            if (params->freq >= nyquist) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "BiquadCombo: freq must be less than Nyquist (%g), got %g", nyquist, params->freq);
                return -1;
            }
            if (!params->has_order || params->order <= 0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "BiquadCombo: order must be > 0, got %d", params->order);
                return -1;
            }
            break;
        ...
        case BIQUAD_COMBO_TYPE_LINKWITZ_RILEY_HIGHPASS:
            ...
            if (!params->has_order || params->order <= 0 || (params->order % 2) != 0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Linkwitz-Riley order must be an even non-zero number, got %d", params->order);
                return -1;
            }
            break;
        case BIQUAD_COMBO_TYPE_TILT:
            if (!params->has_gain) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Tilt: gain must be set");
                return -1;
            }
            if (params->gain <= -100.0 || params->gain >= 100.0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Tilt: gain must be between -100 and 100 dB, got %g", params->gain);
                return -1;
            }
            break;
        case BIQUAD_COMBO_TYPE_FIVE_POINT_PEQ:
            if (!params->has_qls || params->qls <= 0.0 ||
                !params->has_qhs || params->qhs <= 0.0 ||
                !params->has_qp1 || params->qp1 <= 0.0 ||
                !params->has_qp2 || params->qp2 <= 0.0 ||
                !params->has_qp3 || params->qp3 <= 0.0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "FivePointPeq: all Q-values must be > 0");
                return -1;
            }
            if (!params->has_fls || params->fls >= nyquist ||
                !params->has_fhs || params->fhs >= nyquist ||
                !params->has_fp1 || params->fp1 >= nyquist ||
                !params->has_fp2 || params->fp2 >= nyquist ||
                !params->has_fp3 || params->fp3 >= nyquist) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "FivePointPeq: all frequencies must be less than Nyquist (%g)", nyquist);
                return -1;
            }
            if (params->fls <= 0.0 || params->fhs <= 0.0 || params->fp1 <= 0.0 || params->fp2 <= 0.0 || params->fp3 <= 0.0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "FivePointPeq: all frequencies must be > 0");
                return -1;
            }
            break;
        case BIQUAD_COMBO_TYPE_GRAPHIC_EQUALIZER:
            if (!params->gains || params->gains_count == 0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "GraphicEqualizer: gains must be non-empty");
                return -1;
            }
            if (!params->has_freq_min || params->freq_min <= 0.0 || !params->has_freq_max || params->freq_max <= 0.0) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "GraphicEqualizer: min and max frequencies must be > 0");
                return -1;
            }
            if (params->freq_min >= nyquist || params->freq_max >= nyquist) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "GraphicEqualizer: min and max frequencies must be less than Nyquist (%g)", nyquist);
                return -1;
            }
            if (params->freq_min >= params->freq_max) {
                config_error_set(err, CONFIG_ERR_INVALID_FILTER, "GraphicEqualizer: min frequency must be lower than max frequency");
                return -1;
            }
            for (size_t i = 0; i < params->gains_count; i++) {
                double g = params->gains[i];
                if (g < -40.0 || g > 40.0) {
                    config_error_set(err, CONFIG_ERR_INVALID_FILTER, "GraphicEqualizer: gains must be within +- 40 dB, got %g", g);
                    return -1;
                }
            }
            break;
...
int dither_parameters_validate(const dither_parameters_t* params, config_error_t* err) {
    if (!params) return 0;
    if (params->bits < 2) {
        config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Dither bit depth must be at least 2, got %d", params->bits);
        return -1;
    }
    if (params->has_amplitude) {
        if (params->amplitude < 0.0 || params->amplitude > 100.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Dither amplitude must be in [0, 100], got %g", params->amplitude);
            return -1;
        }
    }
    return 0;
...
int lookahead_limiter_parameters_validate(const lookahead_limiter_parameters_t* params, int sample_rate, config_error_t* err) {
    ...
    if (params->attack < 0.0) {
        config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Lookahead Limiter: attack cannot be negative, got %g", params->attack);
        return -1;
    }
    if (params->release < 0.0) {
        config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Lookahead Limiter: release cannot be negative, got %g", params->release);
        return -1;
    }
    ...
    if (attack_samples > (double)sample_rate) {
        config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Lookahead Limiter: attack time cannot be longer than 1 second, got %g samples", attack_samples);
        return -1;
    }
    return 0;
}
...
int volume_parameters_validate(const volume_parameters_t* params, config_error_t* err) {
    ...
    if (params->has_ramp_time) {
        if (params->ramp_time < 0.0) {
            config_error_set(err, CONFIG_ERR_INVALID_FILTER, "Volume ramp time cannot be negative, got %g", params->ramp_time);
            return -1;
        }
    }
    return 0;
...
int filter_config_validate(const filter_config_t* filter, int sample_rate, config_error_t* err) {
    ...
    switch (filter->type) {
        ...
        case FILTER_TYPE_DIFF_EQ: return diff_eq_parameters_validate(&filter->parameters.diff_eq, err);
        case FILTER_TYPE_DITHER: return dither_parameters_validate(&filter->parameters.dither, err);
        case FILTER_TYPE_LIMITER: return limiter_parameters_validate(&filter->parameters.limiter, err);
        case FILTER_TYPE_LOOKAHEAD_LIMITER: return lookahead_limiter_parameters_validate(&filter->parameters.lookahead_limiter, sample_rate, err);
    }
    return 0;
}
