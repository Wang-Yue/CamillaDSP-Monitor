...
static void parse_resampler(const char* js, const jsmntok_t* tokens, int count, int res_val_idx, devices_config_t* devices) {
    ...
    int interp_idx = find_object_key(js, tokens, count, res_val_idx, "interpolation");
    if (interp_idx != -1 && tokens[interp_idx].type == JSMN_STRING) {
        get_tok_string(js, &tokens[interp_idx], res->interpolation, sizeof(res->interpolation));
    ...
    int ac_idx = find_object_key(js, tokens, count, res_val_idx, "apple_complexity");
    ...
    int of_idx = find_object_key(js, tokens, count, res_val_idx, "oversampling_factor");
...
static void parse_capture(const char* js, const jsmntok_t* tokens, int count, int cap_val_idx, devices_config_t* devices) {
    ...
    if (fmt_idx != -1 && tokens[fmt_idx].type == JSMN_STRING) {
        ...
        if (cap->type == AUDIO_BACKEND_TYPE_FILE || cap->type == AUDIO_BACKEND_TYPE_STDIN_OUT) {
...
#if defined(__linux__)
    int soi_idx = find_object_key(js, tokens, count, cap_val_idx, "stop_on_inactive");
    ...
    int lvc_idx = find_object_key(js, tokens, count, cap_val_idx, "link_volume_control");
    if (lvc_idx != -1 && tokens[lvc_idx].type == JSMN_STRING) {
        get_tok_string(js, &tokens[lvc_idx], cap->link_volume_control, sizeof(cap->link_volume_control));
        cap->has_link_volume_control = true;
    }
    int lmc_idx = find_object_key(js, tokens, count, cap_val_idx, "link_mute_control");
    if (lmc_idx != -1 && tokens[lmc_idx].type == JSMN_STRING) {
        get_tok_string(js, &tokens[lmc_idx], cap->link_mute_control, sizeof(cap->link_mute_control));
    ...
    if (sig_idx != -1 && tokens[sig_idx].type == JSMN_OBJECT) {
        ...
        if (sig_type_idx != -1 && tokens[sig_type_idx].type == JSMN_STRING) {
            ...
            get_tok_string(js, &tokens[sig_type_idx], sig_type_str, sizeof(sig_type_str));
...
static void parse_playback(const char* js, const jsmntok_t* tokens, int count, int play_val_idx, devices_config_t* devices) {
    ...
    if (fmt_idx != -1 && tokens[fmt_idx].type == JSMN_STRING) {
        char fmt_str[64];
        get_tok_string(js, &tokens[fmt_idx], fmt_str, sizeof(fmt_str));
        if (play->type == AUDIO_BACKEND_TYPE_FILE || play->type == AUDIO_BACKEND_TYPE_STDIN_OUT) {
...
static int parse_devices(const char* js, const jsmntok_t* tokens, int count, int dev_val_idx, dsp_config_t* config, config_error_t* err) {
    ...
    int era_idx = find_object_key(js, tokens, count, dev_val_idx, "enable_rate_adjust");
    ...
    int st_idx = find_object_key(js, tokens, count, dev_val_idx, "silence_threshold");
    ...
    int sto_idx = find_object_key(js, tokens, count, dev_val_idx, "silence_timeout");
    ...
    int csr_idx = find_object_key(js, tokens, count, dev_val_idx, "capture_samplerate");
    ...
    int vrt_idx = find_object_key(js, tokens, count, dev_val_idx, "volume_ramp_time");
    ...
    int sor_idx = find_object_key(js, tokens, count, dev_val_idx, "stop_on_rate_change");
    ...
    int rmi_idx = find_object_key(js, tokens, count, dev_val_idx, "rate_measure_interval");
    ...
    int wt_idx = find_object_key(js, tokens, count, dev_val_idx, "worker_threads");
    ...
    parse_resampler(js, tokens, count, find_object_key(js, tokens, count, dev_val_idx, "resampler"), dev);
    parse_capture(js, tokens, count, find_object_key(js, tokens, count, dev_val_idx, "capture"), dev);
    parse_playback(js, tokens, count, find_object_key(js, tokens, count, dev_val_idx, "playback"), dev);
    
    return 0;
}

static int parse_pipeline(const char* js, const jsmntok_t* tokens, int count, int pipe_val_idx, dsp_config_t* config, config_error_t* err) {
    ...
    for (int s = 0; s < size; s++) {
        if (tokens[step_idx].type != JSMN_OBJECT) {
            config_error_set(err, CONFIG_ERR_PARSE, "Pipeline step must be an object");
            return -1;
        }
        ...
        if (type_idx != -1 && tokens[type_idx].type == JSMN_STRING) {
            char type_str[64];
            get_tok_string(js, &tokens[type_idx], type_str, sizeof(type_str));
            if (strcmp(type_str, "Filter") == 0) step->type = PIPELINE_STEP_TYPE_FILTER;
            else if (strcmp(type_str, "Mixer") == 0) step->type = PIPELINE_STEP_TYPE_MIXER;
            else if (strcmp(type_str, "Processor") == 0) step->type = PIPELINE_STEP_TYPE_PROCESSOR;
        }
...
static int parse_mixers(const char* js, const jsmntok_t* tokens, int count, int mixers_val_idx, dsp_config_t* config, config_error_t* err) {
    ...
    config->mixers = (named_mixer_config_t*)calloc(size, sizeof(named_mixer_config_t));
    if (!config->mixers) {
        config_error_set(err, CONFIG_ERR_PARSE, "Memory allocation failure");
        return -1;
    }
    ...
    for (int m = 0; m < size; m++) {
        ...
        if (tokens[mixer_val_idx].type != JSMN_OBJECT) {
            config_error_set(err, CONFIG_ERR_PARSE, "Mixer definition must be an object");
            return -1;
        }
        
        mixer_config_t* m_conf = &nm->mixer;
        
        int channels_idx = find_object_key(js, tokens, count, mixer_val_idx, "channels");
        ...
        int mapping_idx = find_object_key(js, tokens, count, mixer_val_idx, "mapping");
        if (mapping_idx != -1 && tokens[mapping_idx].type == JSMN_ARRAY) {
            int map_size = tokens[mapping_idx].size;
            m_conf->mapping = (mixer_mapping_t*)calloc(map_size, sizeof(mixer_mapping_t));
            ...
            for (int mp = 0; mp < map_size; mp++) {
                ...
                if (map_el_idx != -1 && tokens[map_el_idx].type == JSMN_OBJECT) {
                    ...
                    int sources_idx = find_object_key(js, tokens, count, map_el_idx, "sources");
                    if (sources_idx != -1 && tokens[sources_idx].type == JSMN_ARRAY) {
                        int src_size = tokens[sources_idx].size;
                        mapping->sources = (mixer_source_t*)calloc(src_size, sizeof(mixer_source_t));
                        ...
                        for (int s = 0; s < src_size; s++) {
                            ...
                            if (src_el_idx != -1 && tokens[src_el_idx].type == JSMN_OBJECT) {
                                ...
                                int chan_idx = find_object_key(js, tokens, count, src_el_idx, "channel");
                                if (chan_idx != -1 && tokens[chan_idx].type == JSMN_PRIMITIVE) {
                                    src->channel = get_tok_int(js, &tokens[chan_idx]);
                                }
                                
                                int gain_idx = find_object_key(js, tokens, count, src_el_idx, "gain");
                                if (gain_idx != -1 && tokens[gain_idx].type == JSMN_PRIMITIVE) {
                                    src->gain = get_tok_double(js, &tokens[gain_idx]);
                                    src->has_gain = true;
                                }
                                
                                int scale_idx = find_object_key(js, tokens, count, src_el_idx, "scale");
                                if (scale_idx != -1 && tokens[scale_idx].type == JSMN_STRING) {
                                    char scale_str[32];
                                    get_tok_string(js, &tokens[scale_idx], scale_str, sizeof(scale_str));
                                    if (strcasecmp(scale_str, "Linear") == 0) src->scale = GAIN_SCALE_LINEAR;
                                    else src->scale = GAIN_SCALE_DB;
                                } else {
                                    src->scale = GAIN_SCALE_DB;
                                }
                                
                                int inv_idx = find_object_key(js, tokens, count, src_el_idx, "inverted");
                                if (inv_idx != -1 && tokens[inv_idx].type == JSMN_PRIMITIVE) {
                                    src->inverted = get_tok_bool(js, &tokens[inv_idx]);
                                }
                                
                                int smute_idx = find_object_key(js, tokens, count, src_el_idx, "mute");
                                if (smute_idx != -1 && tokens[smute_idx].type == JSMN_PRIMITIVE) {
...
static int parse_filters(const char* js, const jsmntok_t* tokens, int count, int filters_val_idx, dsp_config_t* config, config_error_t* err) {
    ...
    config->filters = (named_filter_config_t*)calloc(size, sizeof(named_filter_config_t));
    if (!config->filters) {
        config_error_set(err, CONFIG_ERR_PARSE, "Memory allocation failure");
        return -1;
    }
    ...
    for (int f = 0; f < size; f++) {
        ...
        if (tokens[filter_val_idx].type != JSMN_OBJECT) {
            config_error_set(err, CONFIG_ERR_PARSE, "Filter definition must be an object");
            return -1;
        }
        ...
        int params_idx = find_object_key(js, tokens, count, filter_val_idx, "parameters");
        if (params_idx != -1 && tokens[params_idx].type == JSMN_OBJECT) {
            switch (f_conf->type) {
                case FILTER_TYPE_GAIN: {
                    ...
                    int scale_idx = find_object_key(js, tokens, count, params_idx, "scale");
                    if (scale_idx != -1 && tokens[scale_idx].type == JSMN_STRING) {
                        char scale_str[32];
                        get_tok_string(js, &tokens[scale_idx], scale_str, sizeof(scale_str));
                        if (strcasecmp(scale_str, "Linear") == 0) gp->scale = GAIN_SCALE_LINEAR;
                        else gp->scale = GAIN_SCALE_DB;
                    } else {
                        gp->scale = GAIN_SCALE_DB;
                    }
                    int inv_idx = find_object_key(js, tokens, count, params_idx, "inverted");
                    if (inv_idx != -1 && tokens[inv_idx].type == JSMN_PRIMITIVE) {
                ...
                case FILTER_TYPE_VOLUME: {
                    ...
                    int rt_idx = find_object_key(js, tokens, count, params_idx, "ramp_time");
                    ...
                    int fader_idx = find_object_key(js, tokens, count, params_idx, "fader");
                    if (fader_idx != -1) {
                ...
                case FILTER_TYPE_LOUDNESS: {
                    ...
                    int ref_idx = find_object_key(js, tokens, count, params_idx, "reference_level");
                    ...
                    int hb_idx = find_object_key(js, tokens, count, params_idx, "high_boost");
                    if (hb_idx != -1 && tokens[hb_idx].type == JSMN_PRIMITIVE) {
                        lp->high_boost = get_tok_double(js, &tokens[hb_idx]);
                        lp->has_high_boost = true;
                    }
                    int lb_idx = find_object_key(js, tokens, count, params_idx, "low_boost");
                    if (lb_idx != -1 && tokens[lb_idx].type == JSMN_PRIMITIVE) {
                        lp->low_boost = get_tok_double(js, &tokens[lb_idx]);
                        lp->has_low_boost = true;
                    }
                    int att_idx = find_object_key(js, tokens, count, params_idx, "attenuate_mid");
                ...
                case FILTER_TYPE_BIQUAD: {
                    ...
                    int btype_idx = find_object_key(js, tokens, count, filter_val_idx, "type");
                    if (btype_idx != -1 && tokens[btype_idx].type == JSMN_STRING) {
                        char bt_str[64];
                        get_tok_string(js, &tokens[btype_idx], bt_str, sizeof(bt_str));
                        if (strcmp(bt_str, "Free") == 0) bp->type = BIQUAD_TYPE_FREE;
                        else if (strcmp(bt_str, "Highpass") == 0) bp->type = BIQUAD_TYPE_HIGHPASS;
                        else if (strcmp(bt_str, "Lowpass") == 0) bp->type = BIQUAD_TYPE_LOWPASS;
                        else if (strcmp(bt_str, "HighpassFO") == 0) bp->type = BIQUAD_TYPE_HIGHPASS_FO;
                        else if (strcmp(bt_str, "LowpassFO") == 0) bp->type = BIQUAD_TYPE_LOWPASS_FO;
                        else if (strcmp(bt_str, "Highshelf") == 0) bp->type = BIQUAD_TYPE_HIGHSHELF;
                        else if (strcmp(bt_str, "Lowshelf") == 0) bp->type = BIQUAD_TYPE_LOWSHELF;
                        else if (strcmp(bt_str, "HighshelfFO") == 0) bp->type = BIQUAD_TYPE_HIGHSHELF_FO;
                        else if (strcmp(bt_str, "LowshelfFO") == 0) bp->type = BIQUAD_TYPE_LOWSHELF_FO;
                        else if (strcmp(bt_str, "Peaking") == 0) bp->type = BIQUAD_TYPE_PEAKING;
                        else if (strcmp(bt_str, "Notch") == 0) bp->type = BIQUAD_TYPE_NOTCH;
                        else if (strcmp(bt_str, "Bandpass") == 0) bp->type = BIQUAD_TYPE_BANDPASS;
                        else if (strcmp(bt_str, "Allpass") == 0) bp->type = BIQUAD_TYPE_ALLPASS;
                        else if (strcmp(bt_str, "AllpassFO") == 0) bp->type = BIQUAD_TYPE_ALLPASS_FO;
                        else if (strcmp(bt_str, "GeneralNotch") == 0) bp->type = BIQUAD_TYPE_GENERAL_NOTCH;
                        else if (strcmp(bt_str, "LinkwitzTransform") == 0) bp->type = BIQUAD_TYPE_LINKWITZ_TRANSFORM;
                    }
                    int freq_idx = find_object_key(js, tokens, count, params_idx, "freq");
                    if (freq_idx != -1 && tokens[freq_idx].type == JSMN_PRIMITIVE) bp->freq = get_tok_double(js, &tokens[freq_idx]);
                    int g_idx = find_object_key(js, tokens, count, params_idx, "gain");
                    if (g_idx != -1 && tokens[g_idx].type == JSMN_PRIMITIVE) bp->gain = get_tok_double(js, &tokens[g_idx]);
                    int q_idx = find_object_key(js, tokens, count, params_idx, "q");
                    if (q_idx != -1 && tokens[q_idx].type == JSMN_PRIMITIVE) bp->q = get_tok_double(js, &tokens[q_idx]);
                    int bw_idx = find_object_key(js, tokens, count, params_idx, "bandwidth");
                    if (bw_idx != -1 && tokens[bw_idx].type == JSMN_PRIMITIVE) bp->bandwidth = get_tok_double(js, &tokens[bw_idx]);
                    int sl_idx = find_object_key(js, tokens, count, params_idx, "slope");
                    if (sl_idx != -1 && tokens[sl_idx].type == JSMN_PRIMITIVE) bp->slope = get_tok_double(js, &tokens[sl_idx]);
                    int a1_idx = find_object_key(js, tokens, count, params_idx, "a1");
                    if (a1_idx != -1 && tokens[a1_idx].type == JSMN_PRIMITIVE) bp->a1 = get_tok_double(js, &tokens[a1_idx]);
                    int a2_idx = find_object_key(js, tokens, count, params_idx, "a2");
                    if (a2_idx != -1 && tokens[a2_idx].type == JSMN_PRIMITIVE) bp->a2 = get_tok_double(js, &tokens[a2_idx]);
                    int b0_idx = find_object_key(js, tokens, count, params_idx, "b0");
                    if (b0_idx != -1 && tokens[b0_idx].type == JSMN_PRIMITIVE) bp->b0 = get_tok_double(js, &tokens[b0_idx]);
                    int b1_idx = find_object_key(js, tokens, count, params_idx, "b1");
                    if (b1_idx != -1 && tokens[b1_idx].type == JSMN_PRIMITIVE) bp->b1 = get_tok_double(js, &tokens[b1_idx]);
                    int b2_idx = find_object_key(js, tokens, count, params_idx, "b2");
                    if (b2_idx != -1 && tokens[b2_idx].type == JSMN_PRIMITIVE) bp->b2 = get_tok_double(js, &tokens[b2_idx]);
                    int fn_idx = find_object_key(js, tokens, count, params_idx, "freq_notch");
                    if (fn_idx != -1 && tokens[fn_idx].type == JSMN_PRIMITIVE) bp->freq_notch = get_tok_double(js, &tokens[fn_idx]);
                    int fp_idx = find_object_key(js, tokens, count, params_idx, "freq_pole");
                    if (fp_idx != -1 && tokens[fp_idx].type == JSMN_PRIMITIVE) bp->freq_pole = get_tok_double(js, &tokens[fp_idx]);
                    int qp_idx = find_object_key(js, tokens, count, params_idx, "q_p");
                    if (qp_idx != -1 && tokens[qp_idx].type == JSMN_PRIMITIVE) bp->q_p = get_tok_double(js, &tokens[qp_idx]);
                    int norm_idx = find_object_key(js, tokens, count, params_idx, "normalize_at_dc");
                    if (norm_idx != -1 && tokens[norm_idx].type == JSMN_PRIMITIVE) bp->normalize_at_dc = get_tok_bool(js, &tokens[norm_idx]);
                    int fa_idx = find_object_key(js, tokens, count, params_idx, "freq_act");
                    if (fa_idx != -1 && tokens[fa_idx].type == JSMN_PRIMITIVE) bp->freq_act = get_tok_double(js, &tokens[fa_idx]);
                    int qa_idx = find_object_key(js, tokens, count, params_idx, "q_act");
                    if (qa_idx != -1 && tokens[qa_idx].type == JSMN_PRIMITIVE) bp->q_act = get_tok_double(js, &tokens[qa_idx]);
                    int ft_idx = find_object_key(js, tokens, count, params_idx, "freq_target");
                    if (ft_idx != -1 && tokens[ft_idx].type == JSMN_PRIMITIVE) bp->freq_target = get_tok_double(js, &tokens[ft_idx]);
                    int qt_idx = find_object_key(js, tokens, count, params_idx, "q_target");
                    if (qt_idx != -1 && tokens[qt_idx].type == JSMN_PRIMITIVE) bp->q_target = get_tok_double(js, &tokens[qt_idx]);
                    break;
                }
                case FILTER_TYPE_DELAY: {
                    ...
                    if (unit_idx != -1 && tokens[unit_idx].type == JSMN_STRING) {
                        ...
                        if (strcmp(u_str, "ms") == 0) dp->unit = DELAY_UNIT_MS;
                        else if (strcmp(u_str, "us") == 0) dp->unit = DELAY_UNIT_US;
                        else if (strcmp(u_str, "samples") == 0) dp->unit = DELAY_UNIT_SAMPLES;
                        else if (strcmp(u_str, "mm") == 0) dp->unit = DELAY_UNIT_MM;
                    } else {
                        dp->unit = DELAY_UNIT_MS;
                    }
                    int sub_idx = find_object_key(js, tokens, count, params_idx, "subsample");
                ...
                case FILTER_TYPE_CONV: {
                    ...
                    int ctype_idx = find_object_key(js, tokens, count, params_idx, "type");
                    if (ctype_idx != -1 && tokens[ctype_idx].type == JSMN_STRING) {
                        char ct_str[32];
                        get_tok_string(js, &tokens[ctype_idx], ct_str, sizeof(ct_str));
                        if (strcmp(ct_str, "Values") == 0) cp->type = CONV_TYPE_VALUES;
                        else if (strcmp(ct_str, "Wav") == 0) cp->type = CONV_TYPE_WAV;
                        else if (strcmp(ct_str, "Raw") == 0) cp->type = CONV_TYPE_RAW;
                        else cp->type = CONV_TYPE_DUMMY;
                    }
                    int val_idx = find_object_key(js, tokens, count, params_idx, "values");
                    ...
                    int file_idx = find_object_key(js, tokens, count, params_idx, "filename");
                    if (file_idx != -1 && tokens[file_idx].type == JSMN_STRING) {
                        get_tok_string(js, &tokens[file_idx], cp->filename, sizeof(cp->filename));
                    }
                    int fmt_idx = find_object_key(js, tokens, count, params_idx, "format");
                    if (fmt_idx != -1 && tokens[fmt_idx].type == JSMN_STRING) {
                        get_tok_string(js, &tokens[fmt_idx], cp->format, sizeof(cp->format));
                    }
                    int chan_idx = find_object_key(js, tokens, count, params_idx, "channel");
                    if (chan_idx != -1 && tokens[chan_idx].type == JSMN_PRIMITIVE) {
                ...
                case FILTER_TYPE_BIQUAD_COMBO: {
                    ...
                    int bctype_idx = find_object_key(js, tokens, count, params_idx, "type");
                    if (bctype_idx != -1 && tokens[bctype_idx].type == JSMN_STRING) {
                        char bct_str[64];
                        get_tok_string(js, &tokens[bctype_idx], bct_str, sizeof(bct_str));
                        if (strcmp(bct_str, "ButterworthHighpass") == 0) bcp->type = BIQUAD_COMBO_TYPE_BUTTERWORTH_HIGHPASS;
                        else if (strcmp(bct_str, "ButterworthLowpass") == 0) bcp->type = BIQUAD_COMBO_TYPE_BUTTERWORTH_LOWPASS;
                        else if (strcmp(bct_str, "LinkwitzRileyHighpass") == 0) bcp->type = BIQUAD_COMBO_TYPE_LINKWITZ_RILEY_HIGHPASS;
                        else if (strcmp(bct_str, "LinkwitzRileyLowpass") == 0) bcp->type = BIQUAD_COMBO_TYPE_LINKWITZ_RILEY_LOWPASS;
                        else if (strcmp(bct_str, "Tilt") == 0) bcp->type = BIQUAD_COMBO_TYPE_TILT;
                        else if (strcmp(bct_str, "FivePointPEQ") == 0) bcp->type = BIQUAD_COMBO_TYPE_FIVE_POINT_PEQ;
                        else if (strcmp(bct_str, "GraphicEqualizer") == 0) bcp->type = BIQUAD_COMBO_TYPE_GRAPHIC_EQUALIZER;
                    }
                    ...
                    int order_idx = find_object_key(js, tokens, count, params_idx, "order");
                    ...
                    int gains_idx = find_object_key(js, tokens, count, params_idx, "gains");
...
int dsp_config_parse_json(const char* json, dsp_config_t** out_config, config_error_t* err) {
    ...
    if (num_tokens < 0) {
        free(config);
        config_error_set(err, CONFIG_ERR_PARSE, "Failed to parse JSON tokens (syntax error)");
        return -1;
    }
