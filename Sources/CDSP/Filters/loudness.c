...
static void recompute_shelves(loudness_filter_t* filter, double volume) {
    double ref = filter->params.has_reference_level ? filter->params.reference_level : -25.0;
    ...
    double low_boost = (filter->params.has_low_boost ? filter->params.low_boost : 10.0) * boost_factor;
    double high_boost = (filter->params.has_high_boost ? filter->params.high_boost : 10.0) * boost_factor;
    ...
    biquad_filter_update_parameters(filter->low_shelf_filter, &lp_cfg, filter->sample_rate);
    ...
    biquad_filter_update_parameters(filter->high_shelf_filter, &hp_cfg, filter->sample_rate);
}
...
loudness_filter_t* loudness_filter_create(const char* name, const loudness_parameters_t* params, int sample_rate, processing_parameters_t* proc_params) {
    loudness_filter_t* filter = (loudness_filter_t*)malloc(sizeof(loudness_filter_t));
...
void loudness_filter_process(loudness_filter_t* filter, mutable_waveform_t waveform, size_t count) {
    ...
    double current_vol = processing_parameters_get_current_volume_for_fader(filter->processing_parameters, filter->params.fader);
    // Recompute coefficients if volume changed significantly
    if (fabs(current_vol - filter->last_volume) > 0.01 || !filter->is_processing_active) {
