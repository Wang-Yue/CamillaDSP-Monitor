...
diffeq_filter_t* diffeq_filter_create(const char* name, const diff_eq_parameters_t* params) {
    ...
    size_t a_cnt = (params && params->a && params->a_count > 0) ? params->a_count : 1;
    size_t b_cnt = (params && params->b && params->b_count > 0) ? params->b_count : 1;
...
void diffeq_filter_process(diffeq_filter_t* filter, mutable_waveform_t waveform, size_t count) {
...
void diffeq_filter_update_parameters(diffeq_filter_t* filter, const filter_config_t* config, int sample_rate) {
    (void)sample_rate;
