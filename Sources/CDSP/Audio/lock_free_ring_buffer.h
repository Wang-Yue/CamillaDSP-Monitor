...
///
///   - **Consume:** call `spsc_audio_ring_buffer_consume(dest, count)` to drain
///   samples.
///     Each sample is delivered to exactly one consumer call.
///     Used by audio capture and playback paths.
///   - **Snapshot:** call `spsc_audio_ring_buffer_read_latest(dest, count)` to
///   copy the
...
/// atomic operations on floating-point types on all targets without locking, so
/// we round-trip through the IEEE-754 bit pattern via `_Atomic uint64_t`.
/// Aligned 64-bit loads and stores are atomic on 64-bit architectures (and
/// ARM/x86 targets), so this is genuinely wait-free.
typedef struct {
