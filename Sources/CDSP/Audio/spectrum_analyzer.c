...
spectrum_analyzer_t* spectrum_analyzer_create(void) {
    ...
    analyzer->db_magnitudes = (float*)calloc(analyzer->fft_n / 2 + 1, sizeof(float));
    ...
    analyzer->plan.frequencies = (float*)calloc(analyzer->out_capacity, sizeof(float));
    analyzer->plan.ranges = (bin_range_t*)calloc(analyzer->out_capacity, sizeof(bin_range_t));
    analyzer->plan.capacity = analyzer->out_capacity;
    analyzer->out_magnitudes = (float*)calloc(analyzer->out_capacity, sizeof(float));

    if (!analyzer->window || !analyzer->data || !analyzer->realp || !analyzer->imagp ||
        !analyzer->magnitudes || !analyzer->db_magnitudes || !analyzer->plan.frequencies ||
...
spectrum_status_t spectrum_analyzer_compute(
    ...
    if (n_bins == 0 || min_freq <= 0.0 || max_freq <= min_freq || samplerate == 0) {
...
#ifdef __APPLE__
    vDSP_vmul(analyzer->data, 1, analyzer->window, 1, analyzer->data, 1, analyzer->fft_n);
#else
    for (size_t i = 0; i < analyzer->fft_n; i++) {
    ...
    vDSP_zvabs(&split_complex1, 1, analyzer->magnitudes + 1, 1, (vDSP_Length)(half_n - 1));
    // Scale the complex bins
    vDSP_vsmul(analyzer->magnitudes + 1, 1, &scale, analyzer->magnitudes + 1, 1, (vDSP_Length)(half_n - 1));

    // Set DC and Nyquist bins
    analyzer->magnitudes[0] = fabsf(analyzer->realp[0]) / (float)analyzer->fft_n;
    analyzer->magnitudes[half_n] = fabsf(analyzer->imagp[0]) / (float)analyzer->fft_n;

    // Threshold the entire magnitudes array to floor_val in-place
    vDSP_vthr(analyzer->magnitudes, 1, &floor_val, analyzer->magnitudes, 1, (vDSP_Length)(half_n + 1));
    ...
    if (n_bins > analyzer->out_capacity) {
        ...
        float* new_freqs = (float*)realloc(analyzer->plan.frequencies, new_cap * sizeof(float));
        bin_range_t* new_ranges = (bin_range_t*)realloc(analyzer->plan.ranges, new_cap * sizeof(bin_range_t));
        float* new_mags = (float*)realloc(analyzer->out_magnitudes, new_cap * sizeof(float));
        ...
        analyzer->plan.samplerate != samplerate)
    {
        ...
        for (size_t i = 0; i < n_bins; i++) {
            ...
            int low_k = (int)floor(low_f * (double)analyzer->fft_n / (double)samplerate);
            int high_k = (int)ceil(high_f * (double)analyzer->fft_n / (double)samplerate);
            int nearest_k = (int)round(center_f * (double)analyzer->fft_n / (double)samplerate);
    ...
    for (size_t i = 0; i < n_bins; i++) {
        ...
        int end = range.high_k < (int)(half_n + 1) ? range.high_k : (int)(half_n + 1);
        int len = end - start;
        ...
        if (len > 0) {
            ...
            for (int k = start; k < end; k++) {
                if (analyzer->db_magnitudes[k] > max_val) max_val = analyzer->db_magnitudes[k];
            }
#endif
