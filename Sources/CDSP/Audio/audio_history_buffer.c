...
audio_history_buffer_t* audio_history_buffer_create(void) {
    audio_history_buffer_t* history = (audio_history_buffer_t*)calloc(1, sizeof(audio_history_buffer_t));
    return history;
}
...
void audio_history_buffer_reset(audio_history_buffer_t* history, size_t channels) {
    ...
    if (channels > 0) {
        history->buffers = (spsc_audio_ring_buffer_t**)calloc(channels, sizeof(spsc_audio_ring_buffer_t*));
        for (size_t ch = 0; ch < channels; ch++) {
            history->buffers[ch] = spsc_audio_ring_buffer_create(AUDIO_HISTORY_BUFFER_CAPACITY);
        }
        if (!history->averaging_scratch) {
            history->averaging_scratch = (float*)calloc(AUDIO_HISTORY_BUFFER_CAPACITY, sizeof(float));
        }
    }
}
...
bool audio_history_buffer_has_data(const audio_history_buffer_t* history) {
    ...
    for (size_t ch = 0; ch < history->channels; ch++) {
        if (history->buffers[ch] && spsc_audio_ring_buffer_get_total_samples_written(history->buffers[ch]) > 0) {
            return true;
        }
    }
    return false;
...
void audio_history_buffer_append(audio_history_buffer_t* history, const audio_chunk_t* chunk) {
    ...
    for (size_t ch = 0; ch < history->channels; ch++) {
        ...
        if (src_ptr && history->buffers[ch]) {
            spsc_audio_ring_buffer_append_converting_double_to_float(history->buffers[ch], src_ptr, valid);
        }
    }
}
...
audio_history_buffer_status_t audio_history_buffer_read_latest(
    ...
    for (size_t ch = 0; ch < history->channels; ch++) {
        ...
        uint64_t w = spsc_audio_ring_buffer_get_total_samples_written(history->buffers[ch]);
        if (w < min_written) {
            min_written = w;
        }
    }
    ...
    if (channel >= 0) {
        bool ok = spsc_audio_ring_buffer_read_latest_at(history->buffers[channel], dest, count, min_written);
    ...
    bool ok = spsc_audio_ring_buffer_read_latest_at(history->buffers[0], dest, count, min_written);
    ...
    for (size_t ch = 1; ch < history->channels; ch++) {
        ok = spsc_audio_ring_buffer_read_latest_at(history->buffers[ch], history->averaging_scratch, count, min_written);
        if (!ok) return AUDIO_HISTORY_BUFFER_OK;
#ifdef __APPLE__
