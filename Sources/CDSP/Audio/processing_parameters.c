...
processing_parameters_t* processing_parameters_create(size_t capture_channels, size_t playback_channels) {
    processing_parameters_t* params = (processing_parameters_t*)calloc(1, sizeof(processing_parameters_t));
    if (!params) return NULL;

    for (int i = 0; i < FADER_COUNT; i++) {
        atomic_double_init(&params->target_volumes[i], PROCESSING_PARAMETERS_DEFAULT_VOLUME);
        atomic_double_init(&params->current_volumes[i], PROCESSING_PARAMETERS_DEFAULT_VOLUME);
    ...
    if (capture_channels > 0) {
        params->capture_signal_peak = (atomic_double_t*)malloc(capture_channels * sizeof(atomic_double_t));
        params->capture_signal_rms = (atomic_double_t*)malloc(capture_channels * sizeof(atomic_double_t));
    ...
    if (playback_channels > 0) {
        params->playback_signal_peak = (atomic_double_t*)malloc(playback_channels * sizeof(atomic_double_t));
        params->playback_signal_rms = (atomic_double_t*)malloc(playback_channels * sizeof(atomic_double_t));
...
bool processing_parameters_is_muted_for_fader(const processing_parameters_t* params, fader_t fader) {
    ...
    return atomic_load_explicit((_Atomic bool*)&params->muted[fader], memory_order_acquire);
}
