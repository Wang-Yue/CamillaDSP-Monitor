...
// High-performance FFTW3 complex DFT backend wrapping Bluestein functionality
// on Linux
#include <fftw3.h>
#include <stdlib.h>
#include <string.h>
...
static void bluestein_fft_execute_wrapper(void* ctx, waveform_t real_in,
  ...
  // Assign real and imaginary parts using GNU C extension real/imag operators
  // (works regardless of fftw_complex definition type)
...
bluestein_fft_t* bluestein_fft_create(size_t n) {
  ...
  fft->plan_forward = fftw_plan_dft_1d(
      (int)n, fft->in_complex, fft->out_complex, FFTW_FORWARD, FFTW_ESTIMATE);
  fft->plan_inverse = fftw_plan_dft_1d(
      (int)n, fft->in_complex, fft->out_complex, FFTW_BACKWARD, FFTW_ESTIMATE);
...
void bluestein_fft_execute(bluestein_fft_t* fft, waveform_t real_in,
                           ...
                           mutable_waveform_t imag_out, bool inverse) {
  if (!fft) return;
  bluestein_fft_execute_wrapper(fft, real_in, imag_in, real_out, imag_out,
                                inverse);
}
