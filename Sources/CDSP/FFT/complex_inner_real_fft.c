...
static void complex_inner_real_fft_forward_wrapper(void* ctx, waveform_t real_in, mutable_waveform_t spec_re, mutable_waveform_t spec_im) {
    complex_inner_real_fft_forward((complex_inner_real_fft_t*)ctx, real_in, spec_re, spec_im);
}

static void complex_inner_real_fft_inverse_wrapper(void* ctx, waveform_t spec_re, waveform_t spec_im, mutable_waveform_t real_out) {
    complex_inner_real_fft_inverse((complex_inner_real_fft_t*)ctx, spec_re, spec_im, real_out);
}
...
complex_inner_real_fft_t* complex_inner_real_fft_create(size_t length, arbitrary_complex_fft_t* inner) {
    ...
    complex_inner_real_fft_t* fft = (complex_inner_real_fft_t*)calloc(1, sizeof(complex_inner_real_fft_t));
    if (!fft) return NULL;
    ...
    if (!fft->twiddle_re || !fft->twiddle_im || !fft->z_re || !fft->z_im || !fft->z_f_re || !fft->z_f_im) {
...
void complex_inner_real_fft_forward(complex_inner_real_fft_t* fft, waveform_t real_in, mutable_waveform_t spec_re, mutable_waveform_t spec_im) {
    ...
    arbitrary_complex_fft_execute(fft->inner, fft->z_re, fft->z_im, fft->z_f_re, fft->z_f_im, false);
...
void complex_inner_real_fft_inverse(complex_inner_real_fft_t* fft, waveform_t spec_re, waveform_t spec_im, mutable_waveform_t real_out) {
    ...
    arbitrary_complex_fft_execute(fft->inner, fft->z_re, fft->z_im, fft->z_f_re, fft->z_f_im, true);
