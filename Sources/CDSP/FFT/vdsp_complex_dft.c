...
#ifdef __APPLE__
    vDSP_DFT_SetupD fwd = vDSP_DFT_zop_CreateSetupD(NULL, (vDSP_Length)n, vDSP_DFT_FORWARD);
    if (!fwd) return NULL;
    vDSP_DFT_SetupD inv = vDSP_DFT_zop_CreateSetupD(fwd, (vDSP_Length)n, vDSP_DFT_INVERSE);
    if (!inv) {
        vDSP_DFT_DestroySetupD(fwd);
        return NULL;
    }
    vdsp_complex_dft_t* dft = (vdsp_complex_dft_t*)malloc(sizeof(vdsp_complex_dft_t));
    if (!dft) {
        vDSP_DFT_DestroySetupD(fwd);
...
#else
    (void)real_in; (void)imag_in; (void)real_out; (void)imag_out; (void)inverse;
#endif
}
