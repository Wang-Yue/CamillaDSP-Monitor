...
async_poly_resampler_t* async_poly_resampler_create(size_t channels, size_t input_rate, size_t output_rate, poly_interpolation_t interpolation, size_t chunk_size, double max_relative_ratio) {
    ...
    async_poly_resampler_t* resampler = (async_poly_resampler_t*)calloc(1, sizeof(async_poly_resampler_t));
    if (!resampler) return NULL;

    resampler->channels = channels;
    resampler->chunk_size = chunk_size;
    resampler->interpolation = interpolation;
    resampler->interpolator_len = (size_t)poly_interpolation_nbr_points(interpolation);
    ...
    double raw_max = ((double)chunk_size - (double)(resampler->interpolator_len + 1) - most_neg_last_index) * max_ratio_abs;
    resampler->max_output_frames = (size_t)(ceil(raw_max)) + 16;

    resampler->start_idx_scratch = (int*)calloc(resampler->max_output_frames, sizeof(int));
...
static inline size_t get_next_output_frames(const async_poly_resampler_t* resampler) {
    // Calculate output size for input
    // — `.floor()`, not `.ceil()`.
    double avg_ratio = 0.5 * resampler->resample_ratio + 0.5 * resampler->target_ratio;
    double raw = ((double)resampler->chunk_size - (double)(resampler->interpolator_len + 1) - resampler->last_index) * avg_ratio;
    return (size_t)floor(raw);
}

static void run_linear(async_poly_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
...
static void run_cubic(async_poly_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
...
static void run_quintic(async_poly_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        for (size_t frame = 0; frame < output_frames; frame++) {
            ...
            double k1 = 6.0 * a - 60.0 * b - 40.0 * c + 120.0 * d - 30.0 * e + 4.0 * f;
            double k0 = 120.0 * c;
...
static void run_septic(async_poly_resampler_t* resampler, size_t output_frames, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        for (size_t frame = 0; frame < output_frames; frame++) {
            ...
            double k7 = -a + 7.0 * b - 21.0 * c + 35.0 * d - 35.0 * e + 21.0 * f - 7.0 * g + h;
            double k6 = 7.0 * a - 42.0 * b + 105.0 * c - 140.0 * d + 105.0 * e - 42.0 * f + 7.0 * g;
            double k5 = -7.0 * a - 14.0 * b + 189.0 * c - 490.0 * d + 595.0 * e - 378.0 * f + 119.0 * g - 14.0 * h;
            double k4 = -35.0 * a + 420.0 * b - 1365.0 * c + 1960.0 * d - 1365.0 * e + 420.0 * f - 35.0 * g;
            double k3 = 56.0 * a - 497.0 * b + 336.0 * c + 1715.0 * d - 3080.0 * e + 1869.0 * f - 448.0 * g + 49.0 * h;
            double k2 = 28.0 * a - 378.0 * b + 3780.0 * c - 6860.0 * d + 3780.0 * e - 378.0 * f + 28.0 * g;
            double k1 = -48.0 * a + 504.0 * b - 3024.0 * c - 1260.0 * d + 5040.0 * e - 1512.0 * f + 336.0 * g - 36.0 * h;
            double k0 = 5040.0 * d;

            // Horner's method
            double val = k0 + x * (k1 + x * (k2 + x * (k3 + x * (k4 + x * (k5 + x * (k6 + x * k7))))));
...
resampler_error_t async_poly_resampler_process(async_poly_resampler_t* resampler, const audio_chunk_t* input, audio_chunk_t* output) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
        ...
        memcpy(dst_ptr + two_n_len, src_ptr, resampler->chunk_size * sizeof(double));
    }
