...
typedef struct audio_resampler {
    ...
    void* impl;
    resampler_error_t (*process)(struct audio_resampler* self, const audio_chunk_t* input, audio_chunk_t* output);
...
/// doesn't match.
static inline resampler_error_t audio_resampler_process(audio_resampler_t* resampler, const audio_chunk_t* input, audio_chunk_t* output) {
...
/// construction).
static inline void audio_resampler_set_relative_ratio(audio_resampler_t* resampler, double multiplier) {
...
static inline double audio_resampler_get_ratio(const audio_resampler_t* resampler) {
    return (resampler && resampler->get_ratio) ? resampler->get_ratio(resampler) : 1.0;
}
...
static inline size_t audio_resampler_get_max_output_frames(const audio_resampler_t* resampler) {
    return (resampler && resampler->get_max_output_frames) ? resampler->get_max_output_frames(resampler) : 0;
}

/// Input frames the resampler expects on every `process` call.
static inline size_t audio_resampler_get_chunk_size(const audio_resampler_t* resampler) {
    return (resampler && resampler->get_chunk_size) ? resampler->get_chunk_size(resampler) : 0;
}

/// Number of channels processed per call.
static inline size_t audio_resampler_get_channels(const audio_resampler_t* resampler) {
    return (resampler && resampler->get_channels) ? resampler->get_channels(resampler) : 0;
}
