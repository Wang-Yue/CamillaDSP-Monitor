...
static void engine_on_chunk_captured_callback(void* ctx, const audio_chunk_t* chunk) {
    audio_history_buffer_t* buf = (audio_history_buffer_t*)ctx;
    if (buf && chunk) audio_history_buffer_append(buf, chunk);
}

static void engine_on_chunk_processed_callback(void* ctx, const audio_chunk_t* chunk) {
    audio_history_buffer_t* buf = (audio_history_buffer_t*)ctx;
    if (buf && chunk) audio_history_buffer_append(buf, chunk);
}
...
void dsp_engine_free(dsp_engine_t* engine) {
    ...
    if (engine->playback_buffer) audio_history_buffer_free(engine->playback_buffer);
    free(engine);
}
...
bool dsp_engine_set_config(dsp_engine_t* engine, const char* json, audio_backend_error_t* err) {
    ...
    logger_info(&logger, "Set config: %s", log_arg_string(json), log_arg_none(), log_arg_none(), log_arg_none());
    ...
    if (config_loader_parse(json, &parsed, &cerr) != 0 || !parsed) {
        if (engine->core) {
            dsp_engine_core_stop(engine->core, (processing_stop_reason_t){ .type = STOP_REASON_NONE });
    ...
    if (engine->core && dsp_engine_core_get_state(engine->core) != PROCESSING_STATE_INACTIVE) {
        if (memcmp(&engine->core->current_config->devices, &parsed->devices, sizeof(devices_config_t)) == 0) {
    ...
    if (!core) {
        ...
        if (err) { err->type = AUDIO_BACKEND_ERR_COMMAND_SEND; snprintf(err->message, sizeof(err->message), "Failed to create core"); }
        return false;
    }
    ...
    for (int i = 0; i < FADER_COUNT; i++) {
        ...
        processing_parameters_set_muted_for_fader(core->processing_params, mute, (fader_t)i);
    }

    audio_history_buffer_reset(engine->capture_buffer, parsed->devices.capture.channels);
    audio_history_buffer_reset(engine->playback_buffer, parsed->devices.playback.channels);
...
void dsp_engine_stop(dsp_engine_t* engine) {
    ...
    if (engine->core && dsp_engine_core_get_state(engine->core) != PROCESSING_STATE_INACTIVE) {
        dsp_engine_core_stop(engine->core, (processing_stop_reason_t){ .type = STOP_REASON_NONE });
        engine->last_stop_reason = (processing_stop_reason_t){ .type = STOP_REASON_NONE };
...
void dsp_engine_set_fader_mute(dsp_engine_t* engine, fader_t fader, bool mute) {
    ...
    if (engine->core && engine->core->processing_params) {
        processing_parameters_set_muted_for_fader(engine->core->processing_params, mute, fader);
    }
}
...
state_update_t dsp_engine_get_status(const dsp_engine_t* engine) {
    ...
    if (engine->core) {
        ...
        const processing_stop_reason_t* r = dsp_engine_core_get_stop_reason(engine->core);
...
vu_levels_t dsp_engine_get_vu_levels(const dsp_engine_t* engine) {
    ...
    if (res.playback_channels > 0) {
        ...
        if (res.playback_rms) processing_parameters_get_playback_signal_rms(p, res.playback_rms, res.playback_channels);
        if (res.playback_peak) processing_parameters_get_playback_signal_peak(p, res.playback_peak, res.playback_channels);
    }
    if (res.capture_channels > 0) {
        ...
        if (res.capture_rms) processing_parameters_get_capture_signal_rms(p, res.capture_rms, res.capture_channels);
        if (res.capture_peak) processing_parameters_get_capture_signal_peak(p, res.capture_peak, res.capture_channels);
    }
    return res;
}
...
audio_samples_t* dsp_engine_get_samples(dsp_engine_t* engine, bool is_capture, size_t n_frames, audio_backend_error_t* err) {
    if (!engine || !engine->core) {
        if (err) { err->type = AUDIO_BACKEND_ERR_ENGINE_NOT_RUNNING; snprintf(err->message, sizeof(err->message), "Engine not running"); }
        return NULL;
    }
    audio_history_buffer_t* buf = is_capture ? engine->capture_buffer : engine->playback_buffer;
    if (!audio_history_buffer_has_data(buf)) {
        if (err) { err->type = AUDIO_BACKEND_ERR_BUFFER_EMPTY; snprintf(err->message, sizeof(err->message), "Buffer empty"); }
        return NULL;
    }
    ...
    if (ch_count == 0) {
        if (err) { err->type = AUDIO_BACKEND_ERR_BUFFER_EMPTY; snprintf(err->message, sizeof(err->message), "No channels"); }
        return NULL;
    }
    ...
    if (!res->channels) { free(res); return NULL; }

    float* tmp = (float*)calloc(n, sizeof(float));
    for (size_t ch = 0; ch < ch_count; ch++) {
        res->channels[ch] = (double*)calloc(n, sizeof(double));
        bool enough = false;
        audio_history_buffer_status_t status = audio_history_buffer_read_latest(buf, tmp, n, (int)ch, &enough);
        if (status != AUDIO_HISTORY_BUFFER_OK) {
            free(tmp);
            dsp_engine_free_samples(res);
            if (err) { err->type = AUDIO_BACKEND_ERR_BUFFER_EMPTY; snprintf(err->message, sizeof(err->message), "Failed to read buffer"); }
            return NULL;
        }
...
static bool iface_get_available_devices(void* ctx, const char* backend, bool is_input, audio_device_t** out_devices, size_t* out_count) {
...
static bool iface_get_device_capabilities(void* ctx, const char* backend, const char* device, bool is_capture, audio_device_descriptor_t** out_desc) {
    if (!ctx || !out_desc) return false;
    *out_desc = dsp_engine_get_device_capabilities(backend, device, is_capture);
    return *out_desc != NULL;
}
static bool iface_get_spectrum(void* ctx, bool is_capture, uint32_t channel, double min_freq, double max_freq, uint32_t n_bins, spectrum_t* out_spec) {
    if (!ctx || !out_spec) return false;
    spectrum_result_t res;
    if (dsp_engine_get_spectrum((dsp_engine_t*)ctx, is_capture, (int)channel, min_freq, max_freq, (size_t)n_bins, &res) != 0) return false;
    ...
    for (size_t i = 0; i < res.count; i++) {
        if (out_spec->frequencies) out_spec->frequencies[i] = (double)res.frequencies[i];
        if (out_spec->magnitudes) out_spec->magnitudes[i] = (double)res.magnitudes[i];
    }
    return true;
}
static bool iface_set_config_json(void* ctx, const char* json_str, char* out_err_msg, size_t err_len) {
    if (!ctx) return false;
