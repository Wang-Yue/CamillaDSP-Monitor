...
engine_processing_loop_t* engine_processing_loop_create(
    ...
    engine_processing_loop_t* loop = (engine_processing_loop_t*)calloc(1, sizeof(engine_processing_loop_t));
    if (!loop) return NULL;
    loop->shared = shared;
...
void engine_processing_loop_free(engine_processing_loop_t* loop) {
    ...
    if (loop->pipeline_queue) {
        pipeline_t* p = NULL;
        while ((p = (pipeline_t*)spsc_queue_dequeue(loop->pipeline_queue)) != NULL) {
            pipeline_free(p);
        }
    ...
    if (loop->update_queue) {
        ...
        while ((u = (pending_update_t*)spsc_queue_dequeue(loop->update_queue)) != NULL) {
...
void pending_update_free(pending_update_t* update) {
    ...
    if (update->processors) {
        for (size_t i = 0; i < update->processors_count; i++) free(update->processors[i]);
        free(update->processors);
    }
    free(update);
}

void engine_processing_loop_set_pipeline(engine_processing_loop_t* loop, pipeline_t* new_pipeline) {
...
void engine_processing_loop_enqueue_update(engine_processing_loop_t* loop, pending_update_t* update) {
...
void engine_processing_loop_run(engine_processing_loop_t* loop) {
    ...
    logger_info(&logger, "Processing thread started", log_arg_none(), log_arg_none(), log_arg_none(), log_arg_none());

    set_realtime_thread_priority("Processing", audio_chunk_get_frames(loop->pipeline_scratch), loop->pipeline_rate);

    size_t pool_cap = loop->shared->processed_queue->capacity + 4;
    round_robin_chunk_pool_t* scratch_pool = round_robin_chunk_pool_create(pool_cap, audio_chunk_get_frames(loop->pipeline_scratch), audio_chunk_get_channels(loop->pipeline_scratch));
    ...
    while (!atomic_load_explicit(&loop->shared->should_stop, memory_order_acquire)) {
        ...
        while ((chunk = (audio_chunk_t*)spsc_queue_dequeue(loop->shared->captured_queue)) != NULL) {
            ...
            while ((update = (pending_update_t*)spsc_queue_dequeue(loop->update_queue)) != NULL) {
                pipeline_update_parameters(loop->active_pipeline, update->config, (const char* const*)update->filters, update->filters_count, (const char* const*)update->mixers, update->mixers_count, (const char* const*)update->processors, update->processors_count);
                pending_update_free(update);
            }
            ...
            if (loop->resampler) {
                ...
                if (rerr != RESAMPLER_OK) {
                    ...
                    snprintf(reason.message, sizeof(reason.message), "Resampler error %d", rerr);
            ...
            // scratch.
            pipeline_t* next_pipeline = (pipeline_t*)spsc_queue_dequeue(loop->pipeline_queue);
            if (next_pipeline) {
                if (loop->active_pipeline) {
                    pipeline_free(loop->active_pipeline);
                }
            ...
            if (perr != PIPELINE_OK) {
                ...
                processing_stop_reason_t reason = { .type = STOP_REASON_UNKNOWN_ERROR };
                snprintf(reason.message, sizeof(reason.message), "Pipeline error %d", perr);
                if (loop->on_stop) loop->on_stop(loop->on_stop_ctx, reason);
                round_robin_chunk_pool_free(scratch_pool);
                return;
            }
    ...
    round_robin_chunk_pool_free(scratch_pool);
    logger_info(&logger, "Processing thread stopped", log_arg_none(), log_arg_none(), log_arg_none(), log_arg_none());
}
