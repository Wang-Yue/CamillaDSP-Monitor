...
engine_capture_loop_t* engine_capture_loop_create(
    ...
    engine_capture_loop_t* loop = (engine_capture_loop_t*)calloc(1, sizeof(engine_capture_loop_t));
    if (!loop) return NULL;
    ...
    silence_counter_init(&loop->silence_counter, silence_threshold_db, silence_timeout_seconds, samplerate, chunk_size);
...
void engine_capture_loop_run(engine_capture_loop_t* loop) {
    ...
    logger_info(&logger, "Capture thread started", log_arg_none(), log_arg_none(), log_arg_none(), log_arg_none());
    ...
    round_robin_chunk_pool_t* chunk_pool = round_robin_chunk_pool_create(pool_cap, loop->chunk_size, loop->channels);
    ...
    while (!atomic_load_explicit(&loop->shared->should_stop, memory_order_acquire)) {
        ...
        if (capture_backend_get_pending_rate_change(loop->capture, &rate)) {
            if (!loop->has_last_observed_pending_rate || rate != loop->last_observed_pending_rate) {
                loop->last_observed_pending_rate = rate;
                loop->has_last_observed_pending_rate = true;
                logger_warn(&logger, "Capture device rate changed to %f Hz; stopping engine", log_arg_double(rate), log_arg_none(), log_arg_none(), log_arg_none());
                processing_stop_reason_t reason = { .type = STOP_REASON_CAPTURE_FORMAT_CHANGE, .format_change_rate = (int)(rate + 0.5) };
                if (loop->on_stop) loop->on_stop(loop->on_stop_ctx, reason);
                break;
            }
        }
        if (playback_backend_get_pending_rate_change(loop->playback, &rate)) {
            if (!loop->has_last_observed_playback_pending_rate || rate != loop->last_observed_playback_pending_rate) {
                loop->last_observed_playback_pending_rate = rate;
                loop->has_last_observed_playback_pending_rate = true;
                logger_warn(&logger, "Playback device rate changed to %f Hz; stopping engine", log_arg_double(rate), log_arg_none(), log_arg_none(), log_arg_none());
                processing_stop_reason_t reason = { .type = STOP_REASON_PLAYBACK_FORMAT_CHANGE, .format_change_rate = (int)(rate + 0.5) };
                if (loop->on_stop) loop->on_stop(loop->on_stop_ctx, reason);
                break;
            }
        }
        ...
        bool got_data = capture_backend_read(loop->capture, loop->chunk_size, chunk, &err);
        if (!got_data) {
            if (err.type != BACKEND_ERROR_NONE) {
                logger_error(&logger, "Capture error: %s", log_arg_string(err.message), log_arg_none(), log_arg_none(), log_arg_none());
            ...
            if (!loop->watchdog_triggered) {
                ...
                double elapsed = (double)(now - loop->watchdog_last_success_ns) / 1000000000.0;
                if (elapsed > loop->watchdog_timeout_seconds) {
                    loop->watchdog_triggered = true;
                    engine_state_machine_set_state(loop->state_machine, PROCESSING_STATE_STALLED);
                    logger_warn(&logger, "Capture device stalled — no data for %fs", log_arg_double(loop->watchdog_timeout_seconds), log_arg_none(), log_arg_none(), log_arg_none());
                }
            }
        ...
        if (loop->watchdog_triggered) {
            loop->watchdog_triggered = false;
            logger_info(&logger, "Capture recovered from stall", log_arg_none(), log_arg_none(), log_arg_none(), log_arg_none());
        }
        ...
        double loudest_peak = processing_parameters_update_capture_levels(loop->processing_params, chunk);
        ...
        processing_state_t desired = silence_counter_update(&loop->silence_counter, loudest_peak);
        processing_state_t current = engine_state_machine_get_state(loop->state_machine);
        if (desired != current) {
            engine_state_machine_set_state(loop->state_machine, desired);
            playback_backend_set_is_paused(loop->playback, (desired == PROCESSING_STATE_PAUSED));
        }
        ...
        // overloaded.
        if (engine_state_machine_get_state(loop->state_machine) != PROCESSING_STATE_PAUSED) {
    ...
    logger_info(&logger, "Capture thread stopped", log_arg_none(), log_arg_none(), log_arg_none(), log_arg_none());
}
