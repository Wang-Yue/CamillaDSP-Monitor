...
static const int supported_carrier_rates[] = {
    176400, 352800, 705600,
    192000, 384000, 768000
};

bool dop_encoder_is_supported_carrier_rate(int rate) {
    size_t count = sizeof(supported_carrier_rates) / sizeof(supported_carrier_rates[0]);
...
static double* build_coeffs(double sample_rate, double cutoff_hz) {
    ...
    for (int ph = 0; ph < DOP_ENC_PHASES; ph++) {
        ...
        for (int m = 0; m < DOP_ENC_SUB_FILTER_TAPS; m++) {
            ...
            int store_idx = ph * DOP_ENC_SUB_FILTER_TAPS + (DOP_ENC_SUB_FILTER_TAPS - 1 - m);
            p[store_idx] = v;
        }
    }
    return p;
...
dop_encoder_t* dop_encoder_create(int channels, double sample_rate, bool output_dop, sdm_filter_t filter_name, double cutoff_hz) {
    ...
    enc->channel_states = (dop_encoder_channel_state_t*)calloc(channels, sizeof(dop_encoder_channel_state_t));
    ...
    for (int ch = 0; ch < channels; ch++) {
        enc->channel_states[ch].modulator = sigma_delta_modulator_create(filter_name, freq);
...
static void encode_channel(dop_encoder_channel_state_t* state, mutable_waveform_t buf, size_t frames, const double* coeffs) {
    if (!buf) return;
...
void dop_encoder_encode(dop_encoder_t* encoder, audio_chunk_t* chunk) {
    ...
    for (int ch = 0; ch < encoder->channels; ch++) {
        encode_channel(&encoder->channel_states[ch], audio_chunk_get_channel(chunk, ch), n, encoder->coeffs);
    }
}
