...
TEST(GeneratorSineCorrectness) {
    ...
    backend_error_t err;
    capture_backend_t* backend = generator_capture_create(&config, 44100, 1024, NULL, &err);
...
TEST(GeneratorSquareCorrectness) {
    ...
    for (size_t f = 0; f < 1024; f++) {
        double expected_val = (sin(expected_phase * 2.0 * M_PI) >= 0.0 ? 0.5 : -0.5);
        ASSERT_NEAR(expected_val, audio_chunk_get_channel(chunk, 0)[f], 1e-6);
        expected_phase += freq_delta;
        if (expected_phase >= 1.0) expected_phase -= 1.0;
    }
...
TEST(GeneratorThrottling) {
    ...
    backend_error_t err;
    capture_backend_t* backend = generator_capture_create(&config, 1000, 100, NULL, &err);
    ASSERT_TRUE(backend != NULL);
    ...
    double elapsed_ms = (double)(end.tv_sec - start.tv_sec) * 1000.0 + (double)(end.tv_nsec - start.tv_nsec) / 1000000.0;
