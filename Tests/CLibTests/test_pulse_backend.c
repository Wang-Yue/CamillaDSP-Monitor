...
TEST(PulsePlaybackBasic) {
    ...
    backend_error_t err;
    playback_backend_t* playback = pulse_playback_create(&play_cfg, 48000, 1024, NULL, &err);
    if (!playback) {
...
TEST(PulseCaptureBasic) {
    ...
    backend_error_t err;
    capture_backend_t* capture = pulse_capture_create(&cap_cfg, 48000, 1024, NULL, &err);
    if (!capture) {
