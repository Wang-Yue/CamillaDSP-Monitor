...
TEST(PipeWirePlaybackBasic) {
    ...
    backend_error_t err;
    playback_backend_t* playback = pipewire_playback_create(&play_cfg, 48000, 1024, NULL, &err);
    if (!playback) {
...
TEST(PipeWireCaptureBasic) {
    ...
    backend_error_t err;
    capture_backend_t* capture = pipewire_capture_create(&cap_cfg, 48000, 1024, NULL, &err);
    if (!capture) {
    ...
    audio_chunk_t* chunk = audio_chunk_create(1024, 2);
    // Use capture_backend_wait with a 100ms timeout to avoid hanging if no audio source is active.
    if (capture_backend_wait(capture, 100)) {
        ...
        printf("No capture data received from PipeWire within 100ms (skipping read test)\n");
    }
