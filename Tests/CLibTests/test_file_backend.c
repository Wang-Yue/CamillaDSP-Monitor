...
TEST(FileBackendRawRoundTrip) {
    ...
    backend_error_t err;
    playback_backend_t* playback = file_playback_create(&play_cfg, 44100, 1024, NULL, &err);
    ...
    capture_backend_t* capture = file_capture_create(&cap_cfg, 44100, 1024, NULL, &err);
    ...
    for (size_t f = 0; f < 100; f++) {
        ASSERT_NEAR((double)f / 100.0, audio_chunk_get_channel(read_chunk, 0)[f], 1e-6);
        ASSERT_NEAR(-(double)f / 100.0, audio_chunk_get_channel(read_chunk, 1)[f], 1e-6);
    }
...
TEST(FileBackendWavRoundTrip) {
    ...
    backend_error_t err;
    playback_backend_t* playback = file_playback_create(&play_cfg, 16000, 1024, NULL, &err);
    ASSERT_TRUE(playback != NULL);
    ASSERT_TRUE(playback_backend_open(playback, &err));
    ...
    capture_backend_t* capture = file_capture_create(&cap_cfg, 0, 1024, NULL, &err);
    ASSERT_TRUE(capture != NULL);
    ASSERT_TRUE(capture_backend_open(capture, &err));
    ...
    for (size_t f = 0; f < 128; f++) {
        ...
        ASSERT_NEAR(expected, audio_chunk_get_channel(read_chunk, 0)[f], 1.0 / 32767.0);
    }
