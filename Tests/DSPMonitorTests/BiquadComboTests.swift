...
@Suite struct BiquadComboTests {
  ...
  private static func compareVecs(_ left: [Double], _ right: [Double], maxdiff: Double) -> Bool {
  ...
  @Test func make_butterworth_5() {
    ...
    let expect: [Double] = [1.62, 0.62, -1.0]
    #expect(q.count == 3)
    #expect(Self.compareVecs(q, expect, maxdiff: 0.01))
  }
  ...
  @Test func make_butterworth_8() {
    ...
    let expect: [Double] = [2.56, 0.9, 0.6, 0.51]
    #expect(q.count == 4)
    #expect(Self.compareVecs(q, expect, maxdiff: 0.01))
  }
  ...
  @Test func make_lr6() {
    ...
    let expect: [Double] = [1.62, 0.62, 1.62, 0.62, 0.5]
    #expect(q.count == 5)
    #expect(Self.compareVecs(q, expect, maxdiff: 0.01))
  }
