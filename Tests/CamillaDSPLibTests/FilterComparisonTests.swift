...
final class FilterComparisonTests: XCTestCase {
  ...
  private func runHarness(_ args: [String]) throws -> Bool {
    ...
    if proc.terminationStatus != 0 {
      ...
      XCTFail("harness exited with \(proc.terminationStatus): \(err)")
      return false
    }
    return true
  }
  ...
  func testBiquad_RawCoeffs_Lowpass1kHz() throws {
  ...
  func testBiquad_RawCoeffs_Highpass5kHz() throws {
  ...
  func testBiquad_RawCoeffs_Peaking2kHz() throws {
  ...
  private func compareBiquad(
    ...
    else { throw XCTSkip("harness binary missing") }
    let ref = try readRaw(from: refPath)

    // Run the same filter in Swift. Use the scalar DF2T path so the output
    // matches camilladsp bit-for-bit (`vDSP.Biquad` reorders operations for
    // SIMD parallelism, which adds ~80 ULP of accumulated rounding noise).
    let coeffs = BiquadCoefficients(b0: b0, b1: b1, b2: b2, a1: a1, a2: a2)
    let filter = BiquadFilter(
      name: label, coefficients: coeffs, sampleRate: Self.sampleRate, useScalarPath: true)
    var swiftOut = input
    var idx = 0
    while idx < swiftOut.count {
      let end = min(idx + Self.chunkSize, swiftOut.count)
      var slice = Array(swiftOut[idx..<end])
      try filter.process(waveform: &slice)
      for (i, v) in slice.enumerated() { swiftOut[idx + i] = v }
      idx = end
    }

    XCTAssertEqual(swiftOut.count, ref.count, "[\(label)] sample count mismatch")
    var maxAbsDiff = 0.0
    var sumSq = 0.0
    ...
    // Scalar DF2T with identical coefficients should be bit-exact — both Swift
    // and camilladsp execute the same `out = s1 + b0*x; s1 = s2 + b1*x - a1*out;
    // s2 = b2*x - a2*out` in the same order on every sample.
    XCTAssertEqual(
      maxAbsDiff, 0.0, "[\(label)] scalar biquad must match camilladsp bit-for-bit")
  }

  // MARK: - Gain

  func testGain_Plus6dB() throws {
    try compareGain(gainDB: 6.0, inverted: false, mute: false, label: "+6dB")
  }

  func testGain_Minus12dB_Inverted() throws {
    try compareGain(gainDB: -12.0, inverted: true, mute: false, label: "-12dB-inv")
  }

  func testGain_Mute() throws {
  ...
  private func compareGain(gainDB: Double, inverted: Bool, mute: Bool, label: String) throws {
    ...
    fp.mute = mute
    fp.scale = .dB
    let filter = GainFilter(name: label, config: FilterConfig(type: .gain, parameters: fp))
    var swiftOut = input
    var idx = 0
    while idx < swiftOut.count {
      let end = min(idx + Self.chunkSize, swiftOut.count)
    ...
    XCTAssertLessThan(maxAbsDiff, 1e-12, "[\(label)] gain output diverges from camilladsp")
  }
  ...
  private func compareVolume(currentVolumeDB: Double, mute: Bool, label: String) throws {
    ...
    let filter = VolumeFilter(
      name: label, rampTimeMs: 0.0, limit: 50.0,
      currentVolume: currentVolumeDB, mute: mute,
      chunkSize: Self.chunkSize, sampleRate: Self.sampleRate,
    ...
    XCTAssertLessThan(maxAbsDiff, 1e-12, "[\(label)] volume output diverges from camilladsp")
  }
  ...
  func testLoudness_BelowReference_Active() throws {
  ...
  func testLoudness_AtReference_NoBoost() throws {
  ...
  func testLoudness_AttenuateMid() throws {
    try compareLoudness(
  ...
  private func compareLoudness(
    ...
    // Build a Swift LoudnessFilter at the matching volume. We flip
    // `BIQUAD_SCALAR=1` so the two internal shelves use the scalar DF2T path
    // and match camilladsp bit-for-bit; the env var is checked on every
    // BiquadFilter construction, so we restore it as soon as we're past init.
    ...
    setenv("BIQUAD_SCALAR", "1", 1)
    let filter = LoudnessFilter(
      name: label, config: FilterConfig(type: .loudness, parameters: lp),
      sampleRate: Self.sampleRate)
    filter.processingParameters = params
    // Trigger `updateShelves()` so the internal biquads' coefficients are
    // populated *while the env var is still set* (LoudnessFilter checks
    // `useScalarPath` per constructed biquad, but the coefficients are
    // installed via `updateParameters` which doesn't rebuild the biquad).
    var primer = [PrcFmt](repeating: 0, count: 8)
    try filter.process(waveform: &primer)
    unsetenv("BIQUAD_SCALAR")
    ...
    XCTAssertLessThan(
      maxAbsDiff, 1e-9, "[\(label)] loudness output diverges from camilladsp")
  }
  ...
  func testMixer_Vs_AnalyticalReference_StereoToMono() throws {
    ...
    let mixer = AudioMixer(name: "stereo-to-mono", config: config, chunkSize: 2048)
    ...
    for i in 0..<1024 {
      ...
      XCTAssertEqual(out.waveforms[0][i], expected, accuracy: 1e-12)
    }
  }

  func testMixer_Vs_AnalyticalReference_LinearScale() throws {
    ...
    let mixer = AudioMixer(name: "linear-scale", config: config, chunkSize: 2048)
  ...
  func testMixer_MutedSource_ProducesSilenceFromThatSource() throws {
    ...
    let mixer = AudioMixer(name: "muted-src", config: config, chunkSize: 2048)
    ...
    for i in 0..<64 {
      XCTAssertEqual(out.waveforms[0][i], l[i], accuracy: 1e-12)
    }
  }
}
