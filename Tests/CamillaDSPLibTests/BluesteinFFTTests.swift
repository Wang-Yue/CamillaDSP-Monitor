...
@Suite struct BluesteinFFTTests {
  ...
  @Test func ForwardMatchesDirectDFT() throws {
  ...
  @Test func InverseMatchesDirectDFT() throws {
  ...
  @Test func BluesteinRealFFTFallbackForPrimeFactors() throws {
    ...
    #expect(abs(recovered[0] - Double(length)) <= 1e-10)
    for i in 1..<length {
      #expect(abs(recovered[i] - 0.0) <= 1e-10)
    }
  }

  /// Non-power-of-2 lengths whose half is `f·2ᵐ` (f ∈ {3, 5, 15}, m ≥ 3)
  /// take the vDSP `DFT_zopD` inner-FFT path inside the complex-inner
  /// real-FFT backend — which is the path the resampler hits at e.g.
  /// 192↔44.1k (halfN=1280=5·2⁸). This test pins the round-trip and
  /// known-bin scaling for that backend so it stays in lock-step with
  /// the `MixedRadixFFT`/`BluesteinFFT` inner alternatives.
  @Test(arguments: [
    48,  // halfN=24=3·2³
    80,  // halfN=40=5·2³
    240,  // halfN=120=15·2³
  ...
  func BluesteinRealFFTVDSPDFTInnerRoundtrip(length: Int) throws {
    ...
    #expect(abs(recovered[0] - Double(length)) <= 1e-9)
    for i in 1..<length {
      #expect(abs(recovered[i]) <= 1e-9)
    }
  }
  ...
  func BluesteinRealFFTPow2VDSPRoundtrip(length: Int) throws {
    ...
    // DC and Nyquist must be real (imag exactly 0 from the unpack path).
    #expect(specIm[0] == 0)
    #expect(specIm[realFFT.spectrumLength - 1] == 0)

    // inverse(flat spectrum) reproduces the impulse scaled by `length`.
    var recovered = [Double](repeating: 0, count: length)
    specRe.withUnsafeBufferPointer { rBuf in
      specIm.withUnsafeBufferPointer { iBuf in
        recovered.withUnsafeMutableBufferPointer { outBuf in
    ...
    #expect(abs(recovered[0] - Double(length)) <= 1e-10)
    for i in 1..<length {
      #expect(abs(recovered[i]) <= 1e-10)
    }

    // Cross-check forward against a known sinusoid: a cosine at bin k
    // should have unscaled real part `length / 2` at bin k (and bin
    // `length-k`, but that's the conjugate fold). This catches sign
    // flips in the un-scale.
    let kBin = max(1, length / 4)
    for n in 0..<length {
      input[n] = cos(2.0 * .pi * Double(kBin) * Double(n) / Double(length))
    }
    input.withUnsafeBufferPointer { inBuf in
      specRe.withUnsafeMutableBufferPointer { rBuf in
        specIm.withUnsafeMutableBufferPointer { iBuf in
    ...
    let expectedRe = Double(length) / 2.0
    #expect(abs(specRe[kBin] - expectedRe) <= 1e-9)
    #expect(abs(specIm[kBin]) <= 1e-9)
  }
}
