// Sigma-delta modulator (DSD encoder core). Lifted from the DoP decoder
// roundtrip test; now lives in CamillaDSPLib so `DoPEncoder` can drive it
// on the playback path. The per-sample `processSample` entry point lets
