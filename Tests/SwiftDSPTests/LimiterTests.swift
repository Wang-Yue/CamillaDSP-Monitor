...
@Suite struct LimiterTests {
  ...
  @Test func test_hard_clip() {
    var waveform: [Double] = [-2.0, -1.0, 0.0, 0.5, 1.5, 2.0]
    ...
    let expected: [Double] = [-0.5, -0.5, 0.0, 0.5, 0.5, 0.5]
  ...
  @Test func test_soft_clip() {
    var waveform: [Double] = [-2.0, -0.5, 0.0, 0.5, 2.0]
    ...
    let expected: [Double] = [-1.0, -0.481481, 0.0, 0.481481, 1.0]
    for (got, exp) in zip(waveform, expected) {
      #expect(Self.isClose(got, exp, maxdiff: 1e-5))
    }
  }
